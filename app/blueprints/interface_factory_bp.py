...
from app.services import interface_factory_service
...
interface_factory_bp = Blueprint("integration", __name__, url_prefix="/api/v1")
...
@interface_factory_bp.route("/programs/<int:program_id>/interfaces", methods=["GET"])
...
@interface_factory_bp.route("/programs/<int:program_id>/interfaces", methods=["POST"])
def create_interface(program_id):
    ...
    iface, err = interface_factory_service.create_interface(program_id, data)
    if err:
...
@interface_factory_bp.route("/interfaces/<int:interface_id>", methods=["GET"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>", methods=["PUT"])
def update_interface(interface_id):
    ...
    iface, svc_err = interface_factory_service.update_interface(iface, data)
    if svc_err:
...
@interface_factory_bp.route("/interfaces/<int:interface_id>", methods=["DELETE"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/status", methods=["PATCH"])
def update_interface_status(interface_id):
    ...
    iface, svc_err = interface_factory_service.update_interface_status(iface, new_status)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/programs/<int:program_id>/interfaces/stats", methods=["GET"])
def interface_stats(program_id):
    ...
    if err:
        return err
    project_id = request.args.get("project_id", type=int) or getattr(g, "project_id", None)
    return jsonify(interface_factory_service.compute_interface_stats(program_id, project_id=project_id))
...
@interface_factory_bp.route("/programs/<int:program_id>/waves", methods=["GET"])
def list_waves(program_id):
    ...
    project_id = request.args.get("project_id", type=int) or getattr(g, "project_id", None)
    waves_query = Wave.query.filter_by(program_id=program_id)
    if project_id is not None:
        waves_query = waves_query.filter(Wave.project_id == project_id)
    waves = waves_query.order_by(Wave.order).all()
    return jsonify([w.to_dict() for w in waves])


@interface_factory_bp.route("/programs/<int:program_id>/waves", methods=["POST"])
def create_wave(program_id):
    ...
    wave, svc_err = interface_factory_service.create_wave(program_id, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/waves/<int:wave_id>", methods=["GET"])
...
@interface_factory_bp.route("/waves/<int:wave_id>", methods=["PUT"])
def update_wave(wave_id):
    ...
    wave, svc_err = interface_factory_service.update_wave(wave, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/waves/<int:wave_id>", methods=["DELETE"])
def delete_wave(wave_id):
    ...
    interface_factory_service.delete_wave(wave)
    err = db_commit_or_error()
    if err:
        return err
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/assign-wave", methods=["PATCH"])
def assign_wave(interface_id):
    ...
    iface, svc_err = interface_factory_service.assign_wave(
        iface,
        wave_id,
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/connectivity-tests", methods=["GET"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/connectivity-tests", methods=["POST"])
def create_connectivity_test(interface_id):
    ...
    test, svc_err = interface_factory_service.create_connectivity_test(interface_id, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/connectivity-tests/<int:test_id>", methods=["GET"])
...
@interface_factory_bp.route("/connectivity-tests/<int:test_id>", methods=["DELETE"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/switch-plans", methods=["GET"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/switch-plans", methods=["POST"])
def create_switch_plan(interface_id):
    ...
    plan, svc_err = interface_factory_service.create_switch_plan(interface_id, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/switch-plans/<int:plan_id>", methods=["PUT"])
def update_switch_plan(plan_id):
    ...
    plan, svc_err = interface_factory_service.update_switch_plan(plan, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/switch-plans/<int:plan_id>", methods=["DELETE"])
...
@interface_factory_bp.route("/switch-plans/<int:plan_id>/execute", methods=["PATCH"])
def execute_switch_plan(plan_id):
    ...
    data = request.get_json(silent=True) or {}
    interface_factory_service.execute_switch_plan(plan, data)
    err = db_commit_or_error()
    if err:
        return err
    return jsonify(plan.to_dict())
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/checklist", methods=["GET"])
...
@interface_factory_bp.route("/interfaces/<int:interface_id>/checklist", methods=["POST"])
def add_checklist_item(interface_id):
    ...
    item, svc_err = interface_factory_service.create_checklist_item(interface_id, data)
    if svc_err:
        return jsonify({"error": svc_err["error"]}), svc_err["status"]
    err = db_commit_or_error()
    if err:
...
@interface_factory_bp.route("/checklist/<int:item_id>", methods=["PUT"])
def update_checklist_item(item_id):
    ...
    interface_factory_service.update_checklist_item(item, data)
    err = db_commit_or_error()
    if err:
        return err
...
@interface_factory_bp.route("/checklist/<int:item_id>", methods=["DELETE"])
