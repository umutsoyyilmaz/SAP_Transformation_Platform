...
external_integration_bp = Blueprint("integrations", __name__, url_prefix="/api/v1")


# Re-export dispatch_webhook_event so existing callers keep working.
dispatch_webhook_event = external_integration_service.dispatch_webhook_event
...
def get_jira_integration(pid):
    ...
    result, err = external_integration_service.get_jira_integration(pid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)


@external_integration_bp.route("/jira-integrations/<int:jid>", methods=["PUT"])
def update_jira_integration(jid):
    """Update a Jira integration."""
    data = request.get_json(silent=True) or {}
    result, err = external_integration_service.update_jira_integration(jid, data)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)


@external_integration_bp.route("/jira-integrations/<int:jid>", methods=["DELETE"])
def delete_jira_integration(jid):
    """Delete a Jira integration."""
    err = external_integration_service.delete_jira_integration(jid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return "", 204
...
def trigger_jira_sync(jid):
    ...
    result, err = external_integration_service.trigger_jira_sync(jid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)
...
def jira_sync_status(jid):
    ...
    result, err = external_integration_service.get_jira_sync_status(jid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)
...
def automation_import():
    ...
    data = request.get_json(silent=True) or {}
    result, err = external_integration_service.create_automation_import(data, _actor())
    if err:
        return jsonify({"error": err["error"]}), err["status"]
...
def automation_import_status(request_id):
    ...
    result, err = external_integration_service.get_automation_status(request_id)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)
...
def list_automation_jobs(pid):
    ...
    result = external_integration_service.list_automation_jobs(pid, status=status, page=page, per_page=per_page)
    return jsonify(result)
...
def process_automation_job(jid):
    ...
    data = request.get_json(silent=True) or {}
    result, err = external_integration_service.process_automation_job(jid, data)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)
...
@external_integration_bp.route("/programs/<int:pid>/webhooks", methods=["GET"])
def list_webhooks(pid):
    """List webhook subscriptions for a program."""
    page = request.args.get("page", 1, type=int)
    per_page = request.args.get("per_page", 20, type=int)
    result = external_integration_service.list_webhooks(pid, page=page, per_page=per_page)
    return jsonify(result)


@external_integration_bp.route("/programs/<int:pid>/webhooks", methods=["POST"])
def create_webhook(pid):
    """Create a webhook subscription."""
    data = request.get_json(silent=True) or {}
    result, err = external_integration_service.create_webhook(pid, data)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result), 201


@external_integration_bp.route("/webhooks/<int:wid>", methods=["GET"])
def get_webhook(wid):
    """Get a single webhook subscription."""
    result, err = external_integration_service.get_webhook(wid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)


@external_integration_bp.route("/webhooks/<int:wid>", methods=["PUT"])
def update_webhook(wid):
    """Update a webhook subscription."""
    data = request.get_json(silent=True) or {}
    result, err = external_integration_service.update_webhook(wid, data)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)


@external_integration_bp.route("/webhooks/<int:wid>", methods=["DELETE"])
def delete_webhook(wid):
    """Delete a webhook subscription."""
    err = external_integration_service.delete_webhook(wid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return "", 204


@external_integration_bp.route("/webhooks/<int:wid>/deliveries", methods=["GET"])
def list_webhook_deliveries(wid):
    """List deliveries for a webhook."""
    page = request.args.get("page", 1, type=int)
    per_page = request.args.get("per_page", 20, type=int)
    result, err = external_integration_service.list_webhook_deliveries(wid, page=page, per_page=per_page)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result)


@external_integration_bp.route("/webhooks/<int:wid>/test", methods=["POST"])
def test_webhook(wid):
    """Send a test ping delivery to the webhook URL."""
    result, err = external_integration_service.test_webhook(wid)
    if err:
        return jsonify({"error": err["error"]}), err["status"]
    return jsonify(result), 201
...
@external_integration_bp.route("/openapi.json", methods=["GET"])
def openapi_spec():
