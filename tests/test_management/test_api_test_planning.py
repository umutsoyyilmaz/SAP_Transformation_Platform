...
class TestPlanTestCase:
    ...
    def test_add_tc_rejects_unknown_planned_tester(self, client, plan, test_case):
        res = client.post(
            f"/api/v1/testing/plans/{plan['id']}/test-cases",
            json={"test_case_id": test_case["id"], "planned_tester_id": 99999},
        )
        assert res.status_code == 400
        assert "planned_tester_id" in res.get_json()["error"]
...
class TestPlanDataSet:
    ...
    def test_link_data_set_from_other_program_rejected(self, client, plan):
        p2 = client.post("/api/v1/programs", json={"name": "Other Program", "methodology": "agile"}).get_json()
        ds2 = client.post(
            "/api/v1/data-factory/test-data-sets",
            json={"name": "Other Program Data", "program_id": p2["id"], "environment": "QAS"},
        ).get_json()

        res = client.post(
            f"/api/v1/testing/plans/{plan['id']}/data-sets",
            json={"data_set_id": ds2["id"]},
        )
        assert res.status_code == 404
...
class TestCycleDataSet:
    ...
    def test_link_data_set_from_other_program_rejected(self, client, cycle):
        p2 = client.post("/api/v1/programs", json={"name": "Cycle Other Program", "methodology": "agile"}).get_json()
        ds2 = client.post(
            "/api/v1/data-factory/test-data-sets",
            json={"name": "Cycle Other Data", "program_id": p2["id"], "environment": "QAS"},
        ).get_json()
