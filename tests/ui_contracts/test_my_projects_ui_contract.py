...
MY_PROJECTS_JS = ROOT / "static/js/views/workspace/my_projects.js"
...
def test_my_projects_view_blocks_unauthorized_open_and_tracks_metrics():
    ...
    assert "destination = 'project-setup'" in src
    assert "App.navigate(destination)" in src
    assert "Open Setup" in src
