...
class TestTraceabilityService:
    ...
    def test_with_backlog(self):
        ...
        req = _make_requirement(p.id)
        bi = BacklogItem(
            program_id=p.id, project_id=p.id, title="BI-1", status="new",
    ...
    def test_full_chain_depth_4(self):
        ...
        req = _make_requirement(p.id)
        bi = BacklogItem(
            program_id=p.id, project_id=p.id, title="BI-2", status="new",
            explore_requirement_id=req.id,
        )
        db.session.add(bi)
        db.session.flush()
        ...
        tc = TestCase(
            program_id=p.id, project_id=p.id, title="TC-1", status="not_run",
        ...
        defect = Defect(
            program_id=p.id, project_id=p.id, title="DEF-1", status="open",
            test_case_id=tc.id,
        )
