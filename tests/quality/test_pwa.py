...
class TestPWAJS:
    ...
    def test_disables_sw_on_localhost(self):
        assert "host === 'localhost' || host === '127.0.0.1'" in self.js
        assert "Local runtime detected; service worker disabled" in self.js
        assert "navigator.serviceWorker.getRegistrations()" in self.js
        assert "caches.keys()" in self.js
