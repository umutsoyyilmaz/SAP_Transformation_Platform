...
def _create_requirement(program_id, title, **overrides):
    ...
    process_level_id = overrides.get("process_level_id")
    if scope_item_id is None:
        l3 = ProcessLevel(
            program_id=program_id,
            project_id=project.id,
            level=3,
            code=overrides.get("scope_item_code", f"L3-INT-{program_id}-{len(title)}"),
            name=overrides.get("scope_item_name", f"{title} Scope"),
            scope_status="in_scope",
            fit_status=overrides.get("fit_status", "gap"),
            scope_item_code=overrides.get("sap_scope_item_code", f"INT{len(title):03d}"[:10]),
            process_area_code=overrides.get("process_area_code", "FI"),
        )
        _db.session.add(l3)
        _db.session.flush()
        scope_item_id = l3.id
        process_level_id = process_level_id or l3.id
    req = ExploreRequirement(
        program_id=program_id,
        project_id=project.id,
        code=overrides.get("code", f"REQ-INT-{program_id}-{len(title)}"),
        title=title,
        type=overrides.get("type", "configuration"),
        requirement_type=overrides.get("req_type", "functional"),
        priority=overrides.get("priority", "P3"),
        status=overrides.get("status", "draft"),
        workshop_id=overrides.get("workshop_id"),
        scope_item_id=scope_item_id,
        process_level_id=process_level_id,
        created_by_id="integration-test",
    )
    _db.session.add(req)
    _db.session.commit()
    return req
