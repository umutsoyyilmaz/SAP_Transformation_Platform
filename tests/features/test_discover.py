...
def test_create_charter_accepts_program_project_types_used_in_ui(client: object) -> None:
    """Discover charter must accept the same project_type values used across setup screens."""
    tenant = _make_tenant("discover-types")
    prog = _make_program(tenant.id)

    for project_type in ("bluefield", "selective_data_transition"):
        res = client.post(
            f"/api/v1/programs/{prog.id}/discover/charter",
            json=_charter_payload(project_type=project_type, project_objective=f"Objective for {project_type}"),
        )
        assert res.status_code in (200, 201), res.get_json()
        assert res.get_json()["project_type"] == project_type
