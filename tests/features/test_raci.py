...
def _make_project(tenant_id: int, program_id: int, code: str, name: str) -> Project:
    project = Project(
        tenant_id=tenant_id,
...
def test_get_raci_matrix_returns_pivot_format(client) -> None:
    ...
    assert all(a["project_id"] is not None for a in data["activities"])
...
def test_get_raci_matrix_filters_to_active_project_scope(client) -> None:
    """GET /programs/<id>/raci?project_id=<pid> returns only project-owned rows."""
    tenant = _make_tenant("raci-project-scope")
    prog = _make_program(tenant.id)
    project_a = _make_project(tenant.id, prog.id, "W1", "Wave 1")
    project_b = _make_project(tenant.id, prog.id, "W2", "Wave 2")

    act_a = _make_activity(tenant.id, prog.id, name="Wave 1 Workshop", project_id=project_a.id)
    mem_a = _make_member(tenant.id, prog.id, name="Alice A", project_id=project_a.id)
    _make_entry(tenant.id, prog.id, act_a.id, mem_a.id, "A", project_id=project_a.id)

    act_b = _make_activity(tenant.id, prog.id, name="Wave 2 Workshop", project_id=project_b.id)
    mem_b = _make_member(tenant.id, prog.id, name="Bob B", project_id=project_b.id)
    _make_entry(tenant.id, prog.id, act_b.id, mem_b.id, "R", project_id=project_b.id)

    res = client.get(f"/api/v1/programs/{prog.id}/raci?project_id={project_a.id}")
    assert res.status_code == 200
    data = res.get_json()

    assert [a["name"] for a in data["activities"]] == ["Wave 1 Workshop"]
    assert [m["name"] for m in data["team_members"]] == ["Alice A"]
    assert data["matrix"].get(str(act_a.id), {}).get(str(mem_a.id)) == "A"
    assert str(act_b.id) not in data["matrix"]
...
def test_upsert_raci_entry_creates_new_record(client) -> None:
    ...
    assert data["project_id"] == act.project_id
...
def test_bulk_import_template_scopes_activities_to_project(client) -> None:
    """POST import-template with project_id stamps created activities to that project."""
    tenant = _make_tenant("raci-template-project")
    prog = _make_program(tenant.id)
    project = _make_project(tenant.id, prog.id, "TMP", "Template Scope")

    res = client.post(
        f"/api/v1/programs/{prog.id}/raci/import-template",
        json={"project_id": project.id},
    )
    assert res.status_code == 201
    assert res.get_json()["created"] > 0

    scoped = client.get(f"/api/v1/programs/{prog.id}/raci?project_id={project.id}")
    assert scoped.status_code == 200
    data = scoped.get_json()
    assert data["activities"]
    assert all(a["project_id"] == project.id for a in data["activities"])
