...
def _make_process_chain(prog_id: int) -> tuple:
    ...
    l1 = ProcessLevel(program_id=prog_id, level=1, code="VC-01", name="Finance", scope_status="in_scope")
    db.session.add(l1)
    db.session.flush()

    l2 = ProcessLevel(program_id=prog_id, level=2, code="PA-FI", name="Accounts Payable",
                      parent_id=l1.id, scope_status="in_scope")
    db.session.add(l2)
    db.session.flush()

    l3 = ProcessLevel(program_id=prog_id, level=3, code="E2E-AP", name="Invoice Processing",
                      parent_id=l2.id, scope_status="in_scope")
    db.session.add(l3)
    db.session.flush()

    l4 = ProcessLevel(program_id=prog_id, level=4, code="L4-INV", name="Invoice Verification",
