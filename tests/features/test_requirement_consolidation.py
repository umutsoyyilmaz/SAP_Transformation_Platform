...
def _default_project_id(program_id: int) -> int:
    row = _db.session.execute(
        __import__("sqlalchemy").text(
            "SELECT id FROM projects WHERE program_id = :pid ORDER BY id LIMIT 1"
        ),
        {"pid": program_id},
    ).first()
...
def _make_req(program_id: int, title: str = "Test Req", **kwargs) -> ExploreRequirement:
    ...
    Sets program_id (FK→programs); project_id (FK→projects) is left NULL.
    """
...
def test_explore_requirement_exposes_canonical_semantic_fields(program):
    """Sprint 1 canonical fields are stored and exposed in to_dict()."""
    req = _make_req(
        program.id,
        title="Canonical semantic fields",
        requirement_class="regulatory",
        delivery_pattern="workflow",
        trigger_reason="control_gap",
        delivery_status="mapped",
    )
    _db.session.flush()

    d = req.to_dict()
    assert d["requirement_class"] == "regulatory"
    assert d["delivery_pattern"] == "workflow"
    assert d["trigger_reason"] == "control_gap"
    assert d["delivery_status"] == "mapped"


def test_explore_requirement_semantic_fields_fallback_to_legacy_values(program):
    """Older rows derive canonical semantics from legacy fields until backfill completes."""
    req = _make_req(
        program.id,
    ...
    d = req.to_dict()
    assert d["requirement_class"] == "functional"
    assert d["delivery_pattern"] == "interface"
    assert d["trigger_reason"] == "partial_fit"
    assert d["delivery_status"] == "not_mapped"
...
def test_migration_script_maps_all_legacy_fields_correctly(program):
    ...
    _db.session.execute(
        __import__("sqlalchemy").text(
            ...
            "(program_id, project_id, tenant_id, code, title, description, req_type, priority, status, source, created_at, updated_at) "
            "VALUES (:pid, :project_id, NULL, :code, :title, :desc, :rtype, :prio, :status, :src, datetime('now'), datetime('now'))"
        ),
        {
            "pid": program.id,
            "project_id": _default_project_id(program.id),
            "code": "REQ-LEG-001",
...
def test_migration_preserves_legacy_requirement_id(program):
    ...
    _db.session.execute(
        sa.text(
            "INSERT INTO requirements "
            "(program_id, project_id, tenant_id, code, title, description, req_type, priority, status, source, created_at, updated_at) "
            "VALUES (:pid, :project_id, NULL, 'REQ-PRES-001', 'Preserve test', '', 'functional', 'must_have', 'draft', 'workshop', datetime('now'), datetime('now'))"
        ),
        {"pid": program.id, "project_id": _default_project_id(program.id)},
    )
    _db.session.flush()
    legacy_id = _db.session.execute(
...
def test_migration_skips_backfill_when_legacy_fk_columns_are_absent(program):
    """Migration remains safe after legacy FK columns have been physically removed."""
    from flask import current_app
    sa = __import__("sqlalchemy")
    ...
    _db.session.execute(
        sa.text(
            "INSERT INTO requirements "
            "(program_id, project_id, tenant_id, code, title, description, req_type, priority, status, source, created_at, updated_at) "
            "VALUES (:pid, :project_id, NULL, 'REQ-BF-001', 'Backfill test', '', 'functional', 'must_have', 'approved', 'workshop', datetime('now'), datetime('now'))"
        ),
        {"pid": program.id, "project_id": project_id},
    )
    _db.session.flush()
    legacy_id = _db.session.execute(
        sa.text("SELECT id FROM requirements WHERE code = 'REQ-BF-001'")
    ).scalar()
    assert legacy_id is not None
    _db.session.commit()
    ...
    stats = migrate(app=current_app._get_current_object(), dry_run=False)
    assert stats["errors"] == 0
    assert stats["backfilled_backlog"] == 0
    assert stats["backfilled_config"] == 0
    assert stats["backfilled_test_cases"] == 0
    assert stats["backfilled_defects"] == 0
...
def test_moscow_summary_via_query(program):
    ...
    rows = _db.session.execute(
        ...
        .where(ExploreRequirement.program_id == program.id)
