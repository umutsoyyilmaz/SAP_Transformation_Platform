...
def program(app):
    ...
    _db.session.add(p)
    _db.session.commit()
    _default_project(p.id)
    return p


@pytest.fixture()
def wave(program):
    project = _default_project(program.id)
    w = Wave(program_id=program.id, project_id=project.id, name="Wave 1 - FI Critical")
    _db.session.add(w)
    _db.session.commit()
    return w
...
def _default_project(program_id):
    ...
    if project is None:
        program = _db.session.get(Program, program_id)
        project = Project(
...
class TestInterfaceList:
    ...
    def test_list_filters_by_project_scope(self, client, program, interface):
        ...
        _db.session.add(foreign_wave)
        _db.session.flush()
        _db.session.add(
            Interface(
                program_id=program.id,
                project_id=foreign_project.id,
                wave_id=foreign_wave.id,
                code="IF-FI-002",
                name="Foreign Scope Interface",
                direction="outbound",
                protocol="rest",
            )
        )
        ...
        rv = client.get(f"/api/v1/programs/{program.id}/interfaces?project_id={default_project.id}")
        data = rv.get_json()["items"]
        assert len(data) == 1
        assert data[0]["id"] == interface.id
...
class TestInterfaceCreate:
    ...
    def test_create_full(self, client, program, wave):
        ...
        assert data["project_id"] == wave.project_id
    ...
    def test_create_accepts_project_scoped_assignee(self, client, program):
        project = _default_project(program.id)
        member = TeamMember(
            tenant_id=program.tenant_id,
            program_id=program.id,
    ...
    def test_create_rejects_foreign_project_assignee(self, client, program):
        default_project = _default_project(program.id)
        foreign_project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
        ...
        member = TeamMember(
            ...
            project_id=foreign_project.id,
            name="Foreign Owner",
            email="foreign-iface@example.com",
            role="Integration Lead",
        )
        _db.session.add(member)
        _db.session.commit()
    ...
    def test_create_rejects_foreign_project_wave(self, client, program):
        default_project = _default_project(program.id)
        foreign_project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
...
class TestInterfaceStats:
    ...
    def test_stats_filter_by_project_scope(self, client, program, interface):
        default_project = _default_project(program.id)
        foreign_project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
        ...
        rv = client.get(f"/api/v1/programs/{program.id}/interfaces/stats?project_id={default_project.id}")
        data = rv.get_json()
        assert data["total"] == 1
        assert data["by_direction"]["inbound"] == 1
...
class TestWaveList:
    ...
    def test_list_filters_by_project_scope(self, client, program, wave):
        default_project = _default_project(program.id)
        foreign_project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
        ...
        rv = client.get(f"/api/v1/programs/{program.id}/waves?project_id={default_project.id}")
        data = rv.get_json()
        assert len(data) == 1
        assert data[0]["id"] == wave.id


class TestWaveCreate:
    def test_create_defaults_to_active_project_scope(self, client, program):
        default_project = _default_project(program.id)
        rv = client.post(
            f"/api/v1/programs/{program.id}/waves",
            json={"name": "Wave 1 - Default Scope"},
        )
        assert rv.status_code == 201
        data = rv.get_json()
        assert data["project_id"] == default_project.id
...
class TestWaveAssignment:
    def test_assign_wave(self, client, program, wave):
        ...
        rv = client.patch(
            ...
            json={"wave_id": wave.id, "project_id": wave.project_id},
        )
        assert rv.status_code == 200
    ...
    def test_assign_rejects_foreign_project_wave(self, client, program, interface):
        default_project = _default_project(program.id)
        foreign_project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
        ...
        rv = client.patch(
            f"/api/v1/interfaces/{interface.id}/assign-wave",
            json={"wave_id": foreign_wave.id, "project_id": default_project.id},
        )
        assert rv.status_code == 400
        assert "active project scope" in rv.get_json()["error"]
