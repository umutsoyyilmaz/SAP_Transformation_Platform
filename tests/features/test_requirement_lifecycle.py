...
class TestADR1ConvertBeforePush:
    ...
    def test_push_to_alm_succeeds_after_convert(self, client):
        ...
        assert r.get_json()["delivery_status"] == "mapped"
...
class TestConvert:
    ...
    def test_convert_creates_backlog_item(self, client):
        ...
        assert body["delivery_status"] == "mapped"
        assert body["backlog_item_id"] is not None
        assert body["config_item_id"] is None
        backlog = _db.session.get(BacklogItem, body["backlog_item_id"])
        assert backlog is not None
        assert backlog.program_id == pid
        assert backlog.project_id == pid
    ...
    def test_convert_creates_config_item(self, client):
        ...
        config = _db.session.get(ConfigItem, body["config_item_id"])
        assert config is not None
        assert config.program_id == pid
...
class TestRequirementSemantics:
    """Sprint 1 semantic guardrails for canonical requirement capture."""

    def test_create_requirement_rejects_standard_fit_observation(self, client):
        pid = _create_program(client)
        r = client.post("/api/v1/explore/requirements", json={
            "project_id": pid,
            "title": "Standard PO creation",
            "type": "functional",
            "fit_status": "fit",
            "created_by_id": "tester",
            "workshop_id": "lifecycle-test-ws",
        })
        assert r.status_code == 400
        assert "standard-fit" in r.get_json()["error"].lower()

    def test_update_requirement_rejects_standard_observation_trigger(self, client):
        pid = _create_program(client)
        req = _create_requirement(client, pid, req_type="development")
        r = client.put(f"/api/v1/explore/requirements/{req['id']}", json={
    ...
    def test_convert_preserves_program_scope_for_non_default_project(self, client):
        pid = _create_program(client)
        ...
        req = _create_requirement(client, project_id, req_type="development")
        _transition(client, req["id"], "submit_for_review")
        _transition(client, req["id"], "approve")
        r = _convert(client, req["id"], project_id, target_type="backlog", wricef_type="interface")
        assert r.status_code == 200
        body = r.get_json()
        backlog = _db.session.get(BacklogItem, body["backlog_item_id"])
        assert backlog.program_id == pid
