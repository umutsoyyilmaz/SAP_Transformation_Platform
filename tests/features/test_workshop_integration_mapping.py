...
class TestFitDecisionFlow:
    ...
    def test_set_fit_via_step_update(self, client, project_id):
        ...
        resp = client.put(
            ...
            json={"fit_decision": "gap", "notes": "Standard doesn't cover this", "project_id": project_id},
        )
...
class TestWorkshopDetailData:
    ...
    def test_workshop_fit_decisions_list(self, client, project_id):
        ...
        ws = _make_workshop(project_id)
        _db.session.commit()
        resp = client.get(f"/api/v1/explore/workshops/{ws.id}/fit-decisions?project_id={project_id}")
        assert resp.status_code == 200
    ...
    def test_workshop_stats(self, client, project_id):
        ...
        l1, l2, l3, l4 = _make_hierarchy(project_id)
        ws = _make_workshop(project_id, status="in_progress", process_area="FI", wave=2, facilitator_id="fac-1")
        ...
        _db.session.add(ExploreOpenItem(
            project_id=project_id,
            workshop_id=ws.id,
        ...
        _db.session.add(ExploreRequirement(
            project_id=project_id,
            workshop_id=ws.id,
            process_step_id=step.id,
            scope_item_id=l3.id,
        ...
        assert data["steps_decided"] == 1
        assert data["total_decisions"] == 1
        assert data["total_open_items"] == 1
        assert data["total_requirements"] == 1
        assert data["total_gaps"] == 1
        assert data["filter_options"]["areas"] == ["FI"]
        assert data["filter_options"]["waves"] == [2]
        assert data["filter_options"]["facilitators"] == ["fac-1"]
        assert data["by_area"]["FI"]["in_progress"] == 1
...
class TestWorkshopFullLifecycle:
    ...
    def test_full_lifecycle(self, client, project_id):
        ...
        resp = client.post(
            ...
            json={"text": "Use standard", "decided_by": "Team", "category": "process", "project_id": project_id},
        )
