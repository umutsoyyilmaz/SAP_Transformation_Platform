    ...
    def test_creates_process_variant_import_records(self):
        ...
        rows = db.session.execute(
            db.select(ProcessVariantImport).where(
                ...
                ProcessVariantImport.program_id == prog.id,
            )
        ).scalars().all()
        assert len(rows) == 2
...
class TestTenantIsolation:
    ...
    def test_list_imports_tenant_isolation(self):
        ...
        db.session.add(ProcessVariantImport(
            tenant_id=1, program_id=prog1.id, variant_id="V001", status="imported"
        ))
        db.session.add(ProcessVariantImport(
            tenant_id=2, program_id=prog2.id, variant_id="V999", status="imported"
        ))
        db.session.commit()
