...
def _make_program(tenant_id: int, name: str = "Timeline Program") -> Program:
    ...
    _db.session.add(p)
    _db.session.flush()
    _make_project(p.id, tenant_id, f"{name[:8].upper()}-DEF", f"{name} Default", is_default=True)
    return p


def _make_project(program_id: int, tenant_id: int, code: str, name: str, *, is_default: bool = False) -> Project:
    p = Project(
        tenant_id=tenant_id,
...
def _make_program_milestone(
    program_id: int,
    tenant_id: int,
    title: str = "Program Milestone",
    planned_date: date | None = None,
    **kwargs,
) -> ProgramMilestone:
...
def test_timeline_includes_program_milestones(client: object) -> None:
    """Timeline milestones list includes consolidated program milestones."""
    tenant = _make_tenant("tl-program-milestones")
    prog = _make_program(tenant.id)
    _make_phase(prog.id, name="Discover")
    _make_program_milestone(
        prog.id,
        tenant.id,
        title="SteerCo Approval",
    ...
    names = {item["name"] for item in milestones}
    assert "SteerCo Approval" in names

    milestone = next(item for item in milestones if item["name"] == "SteerCo Approval")
    assert milestone["type"] == "program_milestone"
    assert milestone["milestone_type"] == "checkpoint"
    assert milestone["record_id"] is not None
...
def test_timeline_project_filter_scopes_phases_and_sprints(client: object) -> None:
    tenant = _make_tenant("tl-project-filter")
    prog = _make_program(tenant.id, name="Project Scoped Timeline")
    project_a = Project.query.filter_by(program_id=prog.id, is_default=True).first()
    assert project_a is not None
    project_b = _make_project(prog.id, tenant.id, "W2", "Wave 2")

    _make_phase(prog.id, name="Wave 1 Explore", tenant_id=tenant.id).project_id = project_a.id
    _make_phase(prog.id, name="Wave 2 Explore", tenant_id=tenant.id).project_id = project_b.id
    _make_sprint(prog.id, name="Wave 1 Sprint", start_date=date(2026, 2, 1), end_date=date(2026, 2, 14)).project_id = project_a.id
    _make_sprint(prog.id, name="Wave 2 Sprint", start_date=date(2026, 3, 1), end_date=date(2026, 3, 14)).project_id = project_b.id
    _db.session.commit()

    res = client.get(f"/api/v1/programs/{prog.id}/timeline?project_id={project_b.id}")
    assert res.status_code == 200
    body = res.get_json()
    assert body["project_id"] == project_b.id
    assert [phase["name"] for phase in body["phases"]] == ["Wave 2 Explore"]
    assert [sprint["name"] for sprint in body["sprints"]] == ["Wave 2 Sprint"]
