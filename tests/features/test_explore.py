...
class TestProcessLevelModel:
    ...
    def test_l3_consolidated_fields(self, project_id):
        ...
        l3.consolidated_decided_at = datetime.now(UTC)
        _db.session.flush()
    ...
    def test_l2_readiness_fields(self, project_id):
        ...
        l2.confirmed_at = datetime.now(UTC)
...
class TestExploreRequirementModel:
    ...
    def test_req_alm_fields(self, project_id):
        ...
        req.alm_id = "ALM-123"
        req.alm_synced = True
        req.alm_synced_at = datetime.now(UTC)
...
class TestProcessLevelAPI:
    ...
    def test_list_tree_supports_server_side_search_with_unfiltered_total(self, client, project_id, hierarchy):
        resp = client.get(f"/api/v1/explore/process-levels?project_id={project_id}&q=Step 1")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["mode"] == "tree"
        assert data["unfiltered_total"] == 4

        def _flatten(items):
            acc = []
            for item in items:
                acc.append(item)
                acc.extend(_flatten(item.get("children") or []))
            return acc

        flat = _flatten(data["items"])
        assert any(item["name"] == "Test Step 1" for item in flat)
        assert all("Step 1" in item["name"] or item["level"] in (1, 2, 3) for item in flat)

    def test_list_tree_supports_server_side_scope_filter(self, client, project_id, hierarchy):
        _, _, l3, l4 = hierarchy
        l4.scope_status = "out_of_scope"
        l4b = ProcessLevel(
        ...
        resp = client.get(f"/api/v1/explore/process-levels?project_id={project_id}&scope_status=out_of_scope")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["mode"] == "tree"
        assert data["unfiltered_total"] == 5
        ...
        flat = _flatten(data["items"])
        l4_rows = [item for item in flat if item["level"] == 4]
        assert len(l4_rows) == 1
        assert l4_rows[0]["scope_status"] == "out_of_scope"

    def test_list_tree_supports_root_first_loading_with_stats(self, client, project_id, hierarchy):
        resp = client.get(f"/api/v1/explore/process-levels?project_id={project_id}&max_depth=2")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["mode"] == "tree"
        assert data["stats"]["l1"] == 1
        assert data["stats"]["l4"] == 1
        assert data["filter_options"]["areas"] == []
        assert len(data["items"]) == 1
        root = data["items"][0]
        assert root["children_loaded"] is True
        assert len(root["children"]) == 1
        assert root["children"][0]["children_loaded"] is False
        assert root["children"][0]["has_children"] is True

    def test_list_tree_supports_child_fetch_for_lazy_expand(self, client, project_id, hierarchy):
        l1, l2, _, _ = hierarchy
        resp = client.get(f"/api/v1/explore/process-levels?project_id={project_id}&parent_id={l2.id}")
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_scope_matrix(self, client, project_id, hierarchy):
        ...
        _db.session.add(WorkshopScopeItem(project_id=project_id, workshop_id=ws.id, process_level_id=l3.id))
        _make_requirement(
            project_id,
            workshop_id=ws.id,
            scope_item_id=l3.id,
        ...
        _make_open_item(
            ...
            code="OI-MATRIX",
            title="Matrix Open Item",
        )
        l4.fit_status = "gap"
        l3.system_suggested_fit = "gap"
        _db.session.commit()

        resp = client.get(f"/api/v1/explore/scope-matrix?project_id={project_id}")
        assert resp.status_code == 200
        data = resp.get_json()
        row = next(item for item in data["items"] if item["id"] == l3.id)
        assert row["workshop_count"] == 1
        assert row["requirement_count"] == 1
        assert row["open_item_count"] == 1
        assert row["workshop_status"] == "draft"
        assert row["effective_fit_status"] == "gap"
        assert row["fit_summary"]["gap"] == 1

    def test_scope_matrix_supports_pagination(self, client, project_id):
        for i in range(30):
            l1 = ProcessLevel(
                project_id=project_id,
                level=1,
        ...
        resp = client.get(f"/api/v1/explore/scope-matrix?project_id={project_id}&page=2&per_page=10")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["page"] == 2
    ...
    def test_seed_from_catalog(self, client, project_id, hierarchy):
        ...
        _db.session.flush()
        resp = client.post(
            f"/api/v1/explore/process-levels/{l3.id}/seed-from-catalog",
            json={"mutation_context": "project_setup"},
        )
    ...
    def test_structural_hierarchy_mutations_require_project_setup_context(self, client, program, hierarchy):
        _, _, l3, _ = hierarchy
        project = Project(
        ...
        delete_res = client.delete(f"/api/v1/explore/process-levels/{l3.id}?confirm=true")
        assert delete_res.status_code == 403

        bulk_res = client.post(
            "/api/v1/explore/process-levels/bulk",
            json={
                "project_id": project.id,
                "levels": [{"level": 1, "code": "L1-BULK", "name": "Bulk Root"}],
            },
        )
        assert bulk_res.status_code == 403

        import_res = client.post(
            "/api/v1/explore/process-levels/import-template",
            json={"project_id": project.id, "selected_l1_codes": ["O2C"]},
        )
        assert import_res.status_code == 403

        seed_res = client.post(f"/api/v1/explore/process-levels/{l3.id}/seed-from-catalog")
        assert seed_res.status_code == 403

        add_child_res = client.post(
            f"/api/v1/explore/process-levels/{l3.id}/children",
            json={"code": "L4-POLICY", "name": "Policy Child"},
        )
        assert add_child_res.status_code == 403

    def test_catalog_seed_requires_project_setup_context(self, client, program):
        project = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
            code="PRJ-CATALOG",
        ...
        res = client.post(
            f"/api/v1/explore/projects/{project.id}/seed-from-catalog",
            json={"tenant_id": project.tenant_id, "modules": ["FI"]},
        )
    ...
    def test_create_process_level_uses_project_scope_with_distinct_project_id(self, client, program):
        p1 = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
            code="PRJ-1",
        ...
        row = _db.session.get(ProcessLevel, data["id"])
        assert row is not None
        assert row.project_id == p2.id
        assert row.program_id == program.id

    def test_list_process_levels_filters_by_project_id_not_program_id(self, client, program):
        p1 = Project(
            tenant_id=program.tenant_id,
            program_id=program.id,
            code="PRJ-L1",
        ...
        level_a = ProcessLevel(project_id=p1.id, program_id=program.id, level=1, code="L1-A", name="Area A")
        level_b = ProcessLevel(project_id=p2.id, program_id=program.id, level=1, code="L1-B", name="Area B")
        _db.session.add_all([level_a, level_b])
        _db.session.commit()

        resp = client.get(f"/api/v1/explore/process-levels?project_id={p2.id}&level=1")
        assert resp.status_code == 200
        items = resp.get_json()["items"]
        assert [item["code"] for item in items] == ["L1-B"]
...
class TestWorkshopAPI:
    ...
    def test_list_workshops_returns_aggregate_counts_without_per_row_queries_contract(self, client, project_id):
        l1, l2, l3, l4 = _make_hierarchy(project_id)
        ws = _make_workshop(project_id, status="in_progress", code="WS-AGG")
        _db.session.add(WorkshopScopeItem(project_id=project_id, workshop_id=ws.id, process_level_id=l3.id))
        step = _make_step(ws.id, l4.id, fit_decision="fit")
        _db.session.add(ExploreDecision(
            project_id=project_id,
        ...
        _make_open_item(project_id, workshop_id=ws.id, code="OI-AGG", title="Agg OI")
        _make_requirement(project_id, workshop_id=ws.id, scope_item_id=l3.id, code="REQ-AGG", title="Agg Req")
        _db.session.commit()

        resp = client.get(f"/api/v1/explore/workshops?project_id={project_id}")
        assert resp.status_code == 200
        items = resp.get_json()["items"]
        row = next(item for item in items if item["id"] == ws.id)
        assert row["steps_total"] == 1
        assert row["fit_count"] == 1
        assert row["decision_count"] == 1
        assert row["oi_count"] == 1
        assert row["req_count"] == 1
        assert row["scope_item_code"] == l3.code
...
class TestWorkshopScopeItemAPI:
    ...
    def test_workshop_full_includes_summary_aggregates(self, client, project_id):
        _, _, l3, l4 = _make_hierarchy(project_id)
        ws = _make_workshop(project_id)
        _db.session.add(WorkshopScopeItem(workshop_id=ws.id, process_level_id=l3.id, sort_order=1))
        _db.session.flush()
        ...
        oi = _make_open_item(project_id, workshop_id=ws.id, process_step_id=step.id, title="OI 1")
        req = _make_requirement(project_id, step.id, workshop_id=ws.id, title="REQ 1")
        _db.session.add(BacklogItem(
        ...
        _db.session.add(RequirementOpenItemLink(requirement_id=req.id, open_item_id=oi.id))
        _db.session.commit()

        resp = client.get(f"/api/v1/explore/workshops/{ws.id}/full?project_id={project_id}")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["summary"]["steps_total"] == 1
        assert data["summary"]["decisions_total"] == 1
        assert data["summary"]["open_items_total"] == 1
        assert data["summary"]["requirements_total"] == 1
        assert data["summary"]["assessed_total"] == 1
        assert data["summary"]["handed_off_total"] == 1
        assert data["summary"]["blocked_total"] == 1
        assert data["summary"]["fit_summary"]["fit"] == 1
