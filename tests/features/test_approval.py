...
def _api_key_headers(monkeypatch, *, key="approval-editor", role="editor") -> dict:
...
class TestWorkflowCRUD:
    def test_create_workflow(self, client, program):
        default_project_id = _default_project_id(program["id"])
        res = client.post(
            f"/api/v1/programs/{program['id']}/approval-workflows",
            json={
                "name": "Plan Approval",
    ...
    def test_list_workflows(self, client, program, workflow):
        ...
        assert all(item["project_id"] == default_project_id for item in items)
...
class TestOperationalRoleGuards:
    def test_operational_permissions_snapshot_uses_backend_matrix(self, client, program):
        res = client.get(f"/api/v1/programs/{program['id']}/testing/operational-permissions")

        assert res.status_code == 200
        data = res.get_json()
        assert data["program_id"] == program["id"]
        assert "admin" in data["current_roles"]
        assert data["permissions"]["approval_submit"]["allowed"] is True
        assert "test_manager" in data["permissions"]["approval_submit"]["allowed_roles"]

    def test_operational_permissions_snapshot_honours_restricted_role(self, client, program, monkeypatch):
        headers = _api_key_headers(monkeypatch)
        res = client.get(
            f"/api/v1/programs/{program['id']}/testing/operational-permissions",
            headers=headers,
        )

        assert res.status_code == 200
        data = res.get_json()
        assert data["permissions"]["approval_submit"]["allowed"] is False
        assert data["permissions"]["approval_decide"]["allowed"] is False

    def test_create_workflow_requires_operational_role(self, client, program, monkeypatch):
        headers = _api_key_headers(monkeypatch)
        res = client.post(
            f"/api/v1/programs/{program['id']}/approval-workflows",
            headers=headers,
        ...
        assert res.get_json()["action"] == "approval_configure"

    def test_submit_requires_operational_role(self, client, workflow, test_case, monkeypatch):
        headers = _api_key_headers(monkeypatch)
        res = client.post(
            "/api/v1/approvals/submit",
            headers=headers,
            json={"entity_type": "test_case", "entity_id": test_case["id"]},
        )
        assert res.status_code == 403
        assert res.get_json()["action"] == "approval_submit"

    def test_decide_requires_operational_role(self, client, workflow, test_case, monkeypatch):
        sub = client.post("/api/v1/approvals/submit", json={
            "entity_type": "test_case",
            "entity_id": test_case["id"],
        }).get_json()
        headers = _api_key_headers(monkeypatch)
        res = client.post(
            f"/api/v1/approvals/{sub['records'][0]['id']}/decide",
            headers=headers,
            json={"decision": "approved"},
        )
        assert res.status_code == 403
        assert res.get_json()["action"] == "approval_decide"
