...
def _make_project(tenant_id: int, program_id: int, code: str = "HC-DEF") -> Project:
    project = Project(
        tenant_id=tenant_id,
        program_id=program_id,
        code=code,
        name=f"{code} Default Project",
        is_default=True,
    )
...
def _make_plan(tenant_id: int, program_id: int, project_id: int, name: str = "Main Plan") -> CutoverPlan:
    plan = CutoverPlan(
        tenant_id=tenant_id,
        program_id=program_id,
...
def _given_plan() -> tuple[int, int]:
    ...
    project = _make_project(t.id, prog.id)
    plan = _make_plan(t.id, prog.id, project.id)
...
def test_approve_cr_sets_approved_by_and_approved_at():
    ...
    cr_row = _db.session.get(PostGoliveChangeRequest, cr_id)
...
def test_tenant_isolation_incident_cross_tenant_404():
    ...
    project_a = _make_project(t_a.id, prog_a.id, "HC-A")
    plan_a = _make_plan(t_a.id, prog_a.id, project_a.id, "Plan A")
    _seed_sla(plan_a.id, "P3", 240, 1440)

    # Tenant B
    t_b = _make_tenant("tenant-b")
    prog_b = _make_program(t_b.id, "Prog B")
    project_b = _make_project(t_b.id, prog_b.id, "HC-B")
    plan_b = _make_plan(t_b.id, prog_b.id, project_b.id, "Plan B")
...
def test_tenant_isolation_cr_cross_tenant_404():
    ...
    project_a = _make_project(t_a.id, prog_a.id, "HC-CRA")
    plan_a = _make_plan(t_a.id, prog_a.id, project_a.id, "CR Plan A")

    # Tenant B
    t_b = _make_tenant("cr-tenant-b")
    prog_b = _make_program(t_b.id, "CR Prog B")
    project_b = _make_project(t_b.id, prog_b.id, "HC-CRB")
    plan_b = _make_plan(t_b.id, prog_b.id, project_b.id, "CR Plan B")
