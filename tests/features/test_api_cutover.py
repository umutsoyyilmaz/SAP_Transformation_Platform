...
def _api_key_headers(monkeypatch, *, key="cutover-editor", role="editor"):
...
class TestCutoverPlan:
    def test_create(self, client):
        ...
        assert plan["project_id"] == default_project.id
    ...
    def test_create_plan_rejects_foreign_project_manager(self, client):
        pid = _program(client)
        default_project = _default_project(pid)
        ...
        member = TeamMember(
            ...
            email="foreign-cutover@example.com",
            role="Cutover Manager",
        )
        db.session.add(member)
        db.session.commit()

        rv = client.post(f"{BASE}/plans", json={
            "program_id": pid,
            "project_id": default_project.id,
            "name": "Scoped Plan",
            "cutover_manager_id": member.id,
        })
        assert rv.status_code == 400
        assert "active project scope" in rv.get_json()["error"]
...
class TestCutoverScopeItem:
    ...
    def test_create_accepts_project_scoped_owner(self, client):
        pid = _program(client)
        ...
        db.session.add(member)
        db.session.commit()
        plan = _plan(client, pid, project_id=project.id)
        rv = client.post(f"{BASE}/plans/{plan['id']}/scope-items", json={
            "name": "Scoped Scope",
            "owner": "Scope Owner",
            "owner_id": member.id,
        })
        assert rv.status_code == 201
        assert rv.get_json()["owner_id"] == member.id
...
class TestRunbookTask:
    ...
    def test_create_rejects_foreign_project_responsible(self, client):
        pid = _program(client)
        default_project = _default_project(pid)
        foreign_project = Project(
            tenant_id=default_project.tenant_id,
        ...
        member = TeamMember(
            ...
            email="foreign-task@example.com",
            role="Consultant",
        )
        db.session.add(member)
        db.session.commit()
        plan = _plan(client, pid, project_id=default_project.id)
        si = _scope_item(client, plan["id"])
        rv = client.post(f"{BASE}/scope-items/{si['id']}/tasks", json={
            "title": "Scoped Task",
...
class TestGoNoGo:
    ...
    def test_create_requires_operational_role(self, client, monkeypatch):
        pid = _program(client)
        plan = _plan(client, pid)
        headers = _api_key_headers(monkeypatch)
        rv = client.post(
            f"{BASE}/plans/{plan['id']}/go-no-go",
            headers=headers,
            json={"criterion": "All tests passed", "source_domain": "test_management"},
        )
        assert rv.status_code == 403
        assert rv.get_json()["action"] == "release_decide"

    def test_update_requires_operational_role(self, client, monkeypatch):
        pid = _program(client)
        plan = _plan(client, pid)
        g = _go_no_go(client, plan["id"])
        headers = _api_key_headers(monkeypatch)
        rv = client.put(
            f"{BASE}/go-no-go/{g['id']}",
            headers=headers,
            json={"verdict": "go"},
        )
        assert rv.status_code == 403
    ...
    def test_delete_requires_operational_role(self, client, monkeypatch):
        pid = _program(client)
        plan = _plan(client, pid)
        g = _go_no_go(client, plan["id"])
        headers = _api_key_headers(monkeypatch)
        rv = client.delete(f"{BASE}/go-no-go/{g['id']}", headers=headers)
        assert rv.status_code == 403
        assert rv.get_json()["action"] == "release_decide"

    def test_seed_requires_operational_role(self, client, monkeypatch):
        pid = _program(client)
        plan = _plan(client, pid)
        headers = _api_key_headers(monkeypatch)
        rv = client.post(f"{BASE}/plans/{plan['id']}/go-no-go/seed", headers=headers)
        assert rv.status_code == 403
        assert rv.get_json()["action"] == "release_decide"
...
class TestHypercare:
    ...
    def test_plan_hypercare_transition(self, client):
        ...
        for st in ["approved", "ready", "executing", "completed", "hypercare"]:
        ...
        signoff = client.post(
            f"/api/v1/programs/{pid}/signoff/hypercare_exit/{plan_id}",
            json={"action": "approved", "approver_id": 1},
        )
        assert signoff.status_code == 201

        rv = client.post(f"{BASE}/plans/{plan_id}/transition", json={"status": "closed"})
        assert rv.status_code == 200, f"Failed at transition to closed: {rv.get_json()}"
