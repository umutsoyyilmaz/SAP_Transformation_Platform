...
@pytest.fixture()
def tenant_project(tenant):
    """Create a concrete project for project membership tests."""
    program = Program(
        tenant_id=tenant.id,
        name="Test Program",
...
class TestUserService:
    ...
    def test_project_membership(self, admin_user, tenant_project):
        from app.services.user_service import assign_to_project, remove_from_project, get_user_projects
        assign_to_project(admin_user.id, tenant_project.id, "project_manager")
        projects = get_user_projects(admin_user.id)
        assert tenant_project.id in projects

        remove_from_project(admin_user.id, tenant_project.id)
        projects = get_user_projects(admin_user.id)
        assert tenant_project.id not in projects
...
class TestAuthAPI:
    ...
    def test_permission_snapshot_endpoint(self, client, tenant, admin_user):
        login_res = client.post("/api/v1/auth/login", json={
            "email": "admin@test-corp.com",
            "password": "SecurePass123!",
            "tenant_slug": "test-corp",
        ...
        res = client.get(
            "/api/v1/auth/permission-snapshot?program_id=12&project_id=34",
            headers={"Authorization": f"Bearer {access_token}"},
        )
        assert res.status_code == 200
        data = res.get_json()
        assert data["program_id"] == 12
        assert data["project_id"] == 34
        assert isinstance(data["roles"], list)
        assert isinstance(data["permissions"], list)

    def test_permission_snapshot_rejects_project_scope_without_program(self, client, tenant, admin_user):
        login_res = client.post("/api/v1/auth/login", json={
            "email": "admin@test-corp.com",
            "password": "SecurePass123!",
            "tenant_slug": "test-corp",
        ...
        res = client.get(
            "/api/v1/auth/permission-snapshot?project_id=34&tenant_id=1",
            headers={"Authorization": f"Bearer {access_token}"},
        )
        assert res.status_code == 400
