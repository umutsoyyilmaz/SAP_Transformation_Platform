...
const ExpUI = (() => {
    ...
    function fitBadge(status, opts = {}) {
        ...
        if (opts.compact) {
            return `<span class="exp-fit-badge exp-fit-badge--compact" style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:${v.bg};color:${v.color};font-size:12px" title="${escAttr(label)}">${icon}</span>`;
        }
    ...
    function metricBar(opts = {}) {
        ...
        const barSegs = segments.map(seg => {
            ...
            if (pct === 0) return '';
            return `<div style="flex:${seg.value || 0};height:${h}px;background:${seg.color};border-radius:${h / 2}px" title="${escAttr(seg.label)}: ${pct}%"></div>`;
        }).join('');
    ...
    function filterGroup(opts = {}) {
        ...
        const chips = (opts.options || []).map(o => {
            ...
            const onclick = opts.onChange ? ` onclick="${escAttr(opts.onChange)}('${escAttr(opts.id)}','${escAttr(o.value)}')"` : '';
        ...
        return `<div class="exp-filter-group" data-group-id="${escAttr(opts.id || '')}" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">${groupLabel}${chips}</div>`;
    }
    ...
    function filterBar(opts = {}) {
        ...
        return `<div class="exp-fb" id="${escAttr(barId)}" data-onchange="${escAttr(opts.onChange || '')}">
            ${row1}
            ${row2}
            ${dropdown}
            ${subDropdowns}
    ...
    function actionButton(opts = {}) {
        ...
        const onclick = opts.onclick ? ` onclick="${escAttr(opts.onclick)}"` : '';
    ...
    function exploreStageNav(opts = {}) {
        const current = opts.current || '';
        const items = [
            { route: 'explore-overview', step: '01', label: 'Explore Overview', eyebrow: 'Discover' },
            { route: 'explore-scope', step: '02', label: 'Scope & Process', eyebrow: 'Design' },
            { route: 'explore-workshops', step: '03', label: 'Workshops', eyebrow: 'Run' },
            { route: 'explore-outcomes', step: '04', label: 'Outcomes', eyebrow: 'Operate' },
            { route: 'explore-traceability', step: '05', label: 'Handoff', eyebrow: 'Govern' },
        ];

        const links = items.map((item) => {
            const active = item.route === current;
            const onclick = active ? '' : ` onclick="App.navigate('${item.route}')"`;
            return `<button type="button" class="explore-stage-link${active ? ' explore-stage-link--active' : ''}"${onclick} ${active ? 'aria-current="page"' : ''}>
        ...
        return `<div class="explore-stage-nav" data-testid="explore-stage-nav">
            <div class="explore-stage-nav__items">${links}</div>
            ${opts.actionsHtml ? `<div class="explore-stage-nav__actions">${opts.actionsHtml}</div>` : ''}
        </div>`;
    }
