...
const ExploreWorkshopDetailView = (() => {
    ...
    function governanceResultFromError(err) {
        return err?.details?.governance || null;
    }

    function governanceMessages(governance) {
        if (!governance) return [];
        const messages = [];
        const sections = [governance.blocks, governance.warnings, governance.infos];
    ...
    function hasGovernanceRule(governance, ruleId) {
        return Array.isArray(governance?.blocks) && governance.blocks.some((block) => block?.rule_id === ruleId);
    }
    ...
    function summary()      { return _ws?.summary || {}; }
    ...
    function renderHeader() {
        ...
        return `${PGBreadcrumb.html([{ label: 'Programs', onclick: 'App.navigate("programs")' }, { label: 'Explore' }, { label: 'Workshops', onclick: 'App.navigate("explore-workshops")' }, { label: w.code || 'Workshop Detail' }])}
        <div class="exp-card" style="margin-bottom:var(--exp-space-md)">
    ...
    function renderKPIStrip() {
        ...
        const fitSummary = totals.fit_summary || {};
        const total = totals.steps_total ?? steps().length;
        const fitC = fitSummary.fit ?? steps().filter(st => st.fit_decision === 'fit').length;
        const partialC = fitSummary.partial_fit ?? steps().filter(st => st.fit_decision === 'partial_fit').length;
        const gapC = fitSummary.gap ?? steps().filter(st => st.fit_decision === 'gap').length;
        const assessed = totals.assessed_total ?? (fitC + partialC + gapC);
        ...
        return `<div class="exp-kpi-strip" style="margin-bottom:var(--exp-space-md)">
            ...
            ${ExpUI.kpiBlock({ value: totals.decisions_total ?? decisions().length,    label: 'Decisions',    accent: 'var(--exp-decision)' })}
            ${ExpUI.kpiBlock({ value: totals.open_items_total ?? openItems().length,    label: 'Open Items',   accent: 'var(--exp-open-item)' })}
            ${ExpUI.kpiBlock({ value: totals.requirements_total ?? requirements().length, label: 'Requirements', accent: 'var(--exp-requirement)' })}
        </div>`;
    }

    function renderOutcomeSummary() {
        const totals = summary();
        const handedOff = totals.handed_off_total ?? requirements().filter((item) => item.backlog_item_id || item.config_item_id).length;
        const blocked = totals.blocked_total ?? requirements().filter((item) => (item.linked_open_items || []).length > 0).length;
        return `<div class="exp-card" style="margin-bottom:var(--exp-space-lg)">
            <div class="exp-card__header" style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
                <div>
                    <div style="font-weight:700">Outcome Summary</div>
                    <div style="font-size:12px;color:var(--sap-text-secondary)">Use this workshop as the output center for decisions, open items, requirements, and backlog handoff.</div>
                </div>
                <div style="display:flex;gap:8px;flex-wrap:wrap">
                    ${ExpUI.actionButton({ label: '+ Decision', variant: 'secondary', size: 'sm', onclick: `ExploreWorkshopDetailView.launchOutcome('decision')` })}
                    ${ExpUI.actionButton({ label: '+ Open Item', variant: 'secondary', size: 'sm', onclick: `ExploreWorkshopDetailView.launchOutcome('openItem')` })}
                    ${ExpUI.actionButton({ label: '+ Requirement', variant: 'primary', size: 'sm', onclick: `ExploreWorkshopDetailView.launchOutcome('requirement')` })}
                    ${ExpUI.actionButton({ label: 'Open Outcomes Hub', variant: 'ghost', size: 'sm', onclick: `App.navigate('explore-outcomes')` })}
                </div>
            </div>
            <div class="exp-card__body">
                <div class="exp-kpi-strip exp-kpi-strip--compact">
                    ${ExpUI.kpiBlock({ value: totals.decisions_total ?? decisions().length, label: 'Decisions', accent: 'var(--exp-decision)' })}
                    ${ExpUI.kpiBlock({ value: totals.open_items_total ?? openItems().length, label: 'Open Items', accent: 'var(--exp-open-item)' })}
                    ${ExpUI.kpiBlock({ value: totals.requirements_total ?? requirements().length, label: 'Requirements', accent: 'var(--exp-requirement)' })}
                    ${ExpUI.kpiBlock({ value: handedOff, label: 'Sent to Backlog', accent: 'var(--exp-fit)' })}
    ...
    function renderTabs() {
        const totals = summary();
        const tabs = [
            { key: 'steps',        label: 'Scope & Outcomes',  count: totals.steps_total ?? steps().length },
            { key: 'decisions',    label: 'Decisions',      count: totals.decisions_total ?? decisions().length },
            { key: 'openItems',    label: 'Open Items',     count: totals.open_items_total ?? openItems().length },
            { key: 'requirements', label: 'Requirements',   count: totals.requirements_total ?? requirements().length },
            { key: 'agenda',       label: 'Agenda',         count: totals.agenda_total ?? agendaItems().length },
            { key: 'attendees',    label: 'Attendees',      count: totals.attendees_total ?? attendees().length },
            { key: 'sessions',     label: 'Sessions',       count: totals.sessions_total ?? sessions().length },
    ...
    function renderProcessStepCard(step) {
        ...
        return `<div class="exp-card" data-step-id="${step.id}" style="border-left:3px solid ${fitColor(fitStatus)}">
    ...
    function launchOutcome(type) {
        _activeTab = 'steps';
        const candidateStepId = _expandedStepId
            || steps().find((step) => ['gap', 'partial_fit', 'pending'].includes(step.fit_decision || 'pending'))?.id
            || steps()[0]?.id;
        if (!candidateStepId) {
            App.toast('No process step available to attach this outcome', 'warning');
            return;
        }
        _expandedStepId = candidateStepId;
        _showInlineForm = { type, stepId: candidateStepId };
        renderPage();
        requestAnimationFrame(() => {
            document.querySelector(`[data-step-id="${candidateStepId}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        });
    }
    ...
    function renderL3ConsolidatedDecision() {
        ...
        const totals = summary();
        const fitCounts = totals.fit_summary || {
            fit: steps().filter(st => st.fit_decision === 'fit').length,
            partial_fit: steps().filter(st => st.fit_decision === 'partial_fit').length,
            gap: steps().filter(st => st.fit_decision === 'gap').length,
            pending: steps().filter(st => !st.fit_decision || st.fit_decision === 'pending').length,
        };
    ...
    async function setFitDecision(stepId, decision) {
        try {
            ...
            if (decision === 'gap' || decision === 'partial_fit') {
                ...
                if (step && await App.confirmDialog({
                    title: 'Create Requirement',
                    message: `Step "${step.name || step.code}" marked as ${decision.replace(/_/g, ' ')}. Would you like to create a requirement?`,
                    confirmLabel: 'Create',
                    variant: 'primary',
                    testId: 'workshop-create-requirement-modal',
                })) {
    ...
    async function startWorkshop() {
        const confirmed = await App.confirmDialog({
            ...
            message: 'Start this workshop? Process steps will be created from scope items.',
            confirmLabel: 'Start',
            variant: 'primary',
            testId: 'workshop-start-modal',
        });
        if (!confirmed) return;
        try {
    ...
    async function completeWorkshop() {
        try {
            ...
            const governance = governanceResultFromError(err);
            const messages = governanceMessages(governance);
            const detailsText = messages.length ? `\n• ${messages.join('\n• ')}` : '';

            // RULE-WC-01: allow explicit override when final-session assessment is incomplete.
            if (hasGovernanceRule(governance, 'RULE-WC-01')) {
                if (await App.confirmDialog({
                    title: 'Force Complete Workshop',
                    message: `Workshop completion is blocked by governance.${detailsText}\n\nForce complete anyway?`,
                    confirmLabel: 'Force Complete',
                    variant: 'primary',
                    testId: 'workshop-force-complete-modal',
                })) {
                    try {
                ...
                App.toast(messages[0] || err.message || 'Failed to complete workshop', 'error');
            }
        }
    }
    ...
    async function reopenWorkshop() {
        const reason = await App.promptDialog({
            title: 'Reopen Workshop',
            message: 'Reason for reopening this workshop is required.',
            label: 'Reason',
            placeholder: 'Enter reopen reason',
            confirmLabel: 'Reopen',
            testId: 'workshop-reopen-modal',
            multiline: true,
            required: true,
        });
        if (reason === null) {
    ...
    async function createDeltaWorkshop() {
        const confirmed = await App.confirmDialog({
            title: 'Create Delta Workshop',
            message: 'Create a delta design workshop based on this completed workshop?',
            confirmLabel: 'Create Delta',
            variant: 'primary',
            testId: 'workshop-delta-modal',
        });
        if (!confirmed) return;
        try {
    ...
    // ── OI Transition ────────────────────────────────────────────────
    function _showOiResolveModal(oiId) {
        App.openModal(`
            <div class="modal">
                ...
                <div class="modal__footer">
                    <button class="btn btn-primary" onclick="ExploreWorkshopDetailView.submitOIResolve('${oiId}')">Resolve</button>
    ...
    function _showOiBlockModal(oiId) {
        App.openModal(`
            <div class="modal">
                <div class="modal__header">
                ...
                <div class="modal__footer">
                    <button class="btn btn-primary" onclick="ExploreWorkshopDetailView.submitOIBlock('${oiId}')">Block</button>
                    <button class="btn" onclick="App.closeModal()">Cancel</button>
                </div>
            </div>
        `);
    ...
    async function transitionOI(oiId, action) {
        if (action === 'resolve') { _showOiResolveModal(oiId); return; }
        if (action === 'block')   { _showOiBlockModal(oiId);   return; }
        try {
    ...
    async function submitOIResolve(oiId) {
        ...
        try {
            await ExploreAPI.openItems.transition(_pid, oiId, { action: 'resolve', resolution });
            App.closeModal();
    ...
    async function submitOIBlock(oiId) {
        ...
        try {
            await ExploreAPI.openItems.transition(_pid, oiId, { action: 'block', blocked_reason });
            App.closeModal();
    ...
    async function deleteAgendaItem(itemId) {
        const confirmed = await App.confirmDialog({
            ...
            confirmLabel: 'Delete',
            testId: 'workshop-delete-agenda-modal',
        });
        if (!confirmed) return;
        try {
    ...
    async function deleteAttendee(attId) {
        const confirmed = await App.confirmDialog({
            ...
            confirmLabel: 'Remove',
            variant: 'primary',
            testId: 'workshop-delete-attendee-modal',
        });
        if (!confirmed) return;
        try {
    ...
    function renderPage() {
        ...
        main.innerHTML = `<div class="explore-page" data-testid="explore-workshop-detail-page">
