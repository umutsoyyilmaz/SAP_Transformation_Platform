...
const HypercareView = (() => {
    ...
    const slaIcon = (inc) => {
        if (inc.sla_resolution_breached) return '<span class="hc-sla-dot" style="color:#ef4444">&#9679;</span>';
        if (inc.sla_response_breached) return '<span class="hc-sla-dot" style="color:#eab308">&#9679;</span>';
        return '<span class="hc-sla-dot" style="color:#22c55e">&#9679;</span>';
    };
    ...
    function _renderEscalationAlerts() {
        ...
        const rows = alerts.map(e => {
            ...
            <div class="hc-esc-alert ${isCritical ? 'hc-esc-alert--critical' : ''}">
                <span class="badge badge--${isCritical ? 'red' : 'orange'} hc-esc-alert__level">${e.escalation_level}</span>
                <span class="hc-esc-alert__target">${e.escalated_to || 'Unknown'} &mdash; ${e.trigger_type || ''}</span>
                <span class="hc-esc-alert__time">${_timeAgo(e.created_at)}</span>
        ...
        <div class="hc-esc-panel">
            <div class="hc-esc-panel__header">
                Escalation Alerts
                <span class="badge badge--red hc-esc-panel__count">${alerts.length}</span>
            </div>
            ${rows}
        </div>`;
    }
    ...
    function _renderWarRoomOverview() {
        ...
        const rows = activeRooms.map(wr => {
            ...
            <div class="hc-wr-row ${borderClass}" onclick="HypercareView.switchTab('war-rooms')">
                ...
                <span class="badge badge--${wr.status === 'active' ? 'blue' : 'gray'} hc-wr-row__badge">${wr.status}</span>
                <span class="hc-wr-row__stats">
                    ${wr.open_incidents} open${hasP1 ? ` <span class="hc-wr-row__priority" style="color:#dc2626">(${wr.open_p1} P1)</span>` : ''}${hasP2 ? ` <span class="hc-wr-row__priority" style="color:#e76500">(${wr.open_p2} P2)</span>` : ''}
                </span>
                ${wr.cr_count > 0 ? `<span class="hc-wr-row__meta">${wr.cr_count} CRs</span>` : ''}
            </div>`;
        }).join('');

        return `
        <div class="hc-wr-overview">
            <div class="hc-wr-overview__eyebrow">Active War Rooms</div>
            ${rows}
        </div>`;
    }
    ...
    function _renderIncidentRow(inc) {
        ...
        const escLevel = inc.current_escalation_level
            ? `<span class="badge badge--red hc-incident-row__flag">${inc.current_escalation_level}</span>`
            : '';
        const wrBadge = inc.war_room_id
            ? `<span class="badge badge--blue hc-incident-row__flag">WR</span>`
            : '';
        return `
    ...
    function _renderIncidentTab() {
        ...
        return `
        <div class="section-header hc-section-header">
    ...
    function _renderCRRow(cr) {
        ...
        const wrBadge = cr.war_room_id
            ? `<span class="badge badge--blue hc-cr-row__flag">WR</span>`
            : '';
        return `
        <tr data-cr-id="${cr.id}">
    ...
    function _renderWarRoomsTab() {
        if (_warRooms.length === 0) {
            return `
            <div class="hc-empty-panel">
                No war rooms created yet.
                <button class="btn btn--xs btn--primary hc-empty-panel__cta" onclick="HypercareView.showCreateWarRoom()">
                    + Create War Room
                </button>
            </div>`;
        }
        ...
        const cards = _warRooms.map(wr => {
            ...
            <div class="hc-wr-card" onclick="HypercareView.openWarRoom(${wr.id})">
                <div class="hc-wr-card__header">
                    ...
                    <span class="badge ${statusBadgeClass} hc-wr-card__badge">${wr.status}</span>
                </div>
                <div class="hc-wr-card__body">
                    <span class="badge ${priorityClass} hc-wr-card__priority">${wr.priority}</span>
                    ${wr.affected_module ? `<span class="hc-wr-card__meta">${wr.affected_module}</span>` : ''}
                    ${wr.war_room_lead ? `<span class="hc-wr-card__meta">Lead: ${wr.war_room_lead}</span>` : ''}
                </div>
    ...
    function _renderExitReadiness() {
        if (_exitCriteria.length === 0) {
            ...
            <section class="view-section">
                ...
                <div class="hc-empty-panel">
                    ...
                    <button class="btn btn--xs btn--secondary hc-empty-panel__cta" onclick="HypercareView.seedExitCriteria()">
        ...
        const criteriaGrid = _exitCriteria.map(c => {
            ...
            const mandatoryBadge = c.is_mandatory
                ? '<span class="hc-exit-required">*</span>' : '';
            return `
            <div class="hc-exit-item">
                <span class="hc-exit-icon ${iconClass}">${iconChar}</span>
                <span class="hc-exit-item__copy">${c.name}${mandatoryBadge}</span>
                ${c.current_value !== null && c.current_value !== undefined
                    ? `<span class="hc-exit-item__value">${c.current_value}</span>` : ''}
            </div>`;
        }).join('');
        ...
        <section class="view-section">
            <div class="hc-exit-header">
                <div class="hc-exit-header__left">
                    <h3 class="hc-exit-title">Exit Readiness</h3>
            ...
            <div class="hc-exit-footnote">* = mandatory for exit</div>
        </section>`;
    }
    ...
    function _renderHTML() {
        ...
        <div class="hc-war-room">
            <div class="pg-view-header hc-page-header">
                <h2>Hypercare War Room</h2>
                <div class="hc-page-header__actions">
    ...
    async function openIncident(incidentId) {
        ...
        const wrSelect = activeWRs.length > 0
            ? `<div class="hc-incident-detail__assign">
                <label class="hc-incident-detail__assign-label">Assign to War Room</label>
                <select class="hc-incident-detail__assign-select" onchange="HypercareView._assignIncidentWarRoom(${inc.id}, this.value)">
        ...
        const escHTML = escalations.length > 0
            ? `<div class="hc-esc-history">
                <h4 class="hc-esc-history__title">Escalation History</h4>
                ${escalations.map(e => `
                    <div class="hc-esc-history__row">
                        <span class="badge badge--${['L3','vendor','management'].includes(e.escalation_level) ? 'red' : 'orange'} hc-esc-alert__level">${e.escalation_level}</span>
                        <span>${e.escalated_to || ''}</span>
                        <span class="hc-esc-history__time">${_timeAgo(e.created_at)}</span>
                        ${e.acknowledged_at ? '<span class="hc-esc-history__ack">Ack</span>' : ''}
                    </div>`).join('')}
               </div>`
            : '';

        const lessonsHTML = lessons.length > 0
            ? `<div class="hc-lessons">
                <h4 class="hc-lessons__title">Similar Lessons</h4>
                ${lessons.map(l => `
                    <div class="hc-lessons__row">
                        ${l.title || 'Untitled'} <span class="hc-lessons__meta">${l.category || ''}</span>
                    </div>`).join('')}
               </div>`
            : '';
        ...
        const html = `<div class="modal">
            ...
            <div class="modal__body">
                <div class="detail-row hc-incident-detail__row">
                ...
                <p>${inc.description || '\u2014'}</p>
                ${inc.sla_response_deadline ? `<p class="hc-incident-detail__deadline">Response deadline: ${new Date(inc.sla_response_deadline).toLocaleString()}</p>` : ''}
                ${inc.sla_resolution_deadline ? `<p class="hc-incident-detail__deadline">Resolution deadline: ${new Date(inc.sla_resolution_deadline).toLocaleString()}</p>` : ''}
                ${inc.root_cause ? `<p class="hc-incident-detail__root-cause"><strong>Root Cause:</strong> ${inc.root_cause}</p>` : ''}
                ${wrSelect}
                <div class="incident-actions hc-incident-detail__actions">
