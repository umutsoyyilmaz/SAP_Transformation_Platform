const TestCaseDetailView = (() => {
    ...
    async function _ensureExecutionRows() {
        ...
        const payload = await API.get(`/testing/catalog/${_caseId}/execution-history`).catch(() => ({ items: [] }));
        _executionRows = payload?.items || payload || [];
    ...
    function _renderShell() {
        ...
        main.innerHTML = `
            <div class="tm-case-detail__crumb">
                <div id="tcBreadcrumb"></div>
            </div>
            <div class="tm-card tm-case-detail__header">
                <div class="tm-case-detail__header-top">
                    <div class="tm-case-detail__title-row">
                        <h1 class="tm-case-detail__title">${_esc(code)}</h1>
                        <span class="tm-case-detail__subtitle">${_esc(_tc.title || 'Untitled Test Case')}</span>
                    </div>
                    <div id="tcHeaderToolbar"></div>
                </div>
                <div class="tm-case-detail__meta">
                    ...
                    <span class="tm-case-detail__meta-copy">Module: <strong>${_esc(current.module || '-')}</strong></span>
                    <span class="tm-case-detail__meta-copy">Updated: <strong>${_esc((current.updated_at || current.created_at || '-').slice(0, 10))}</strong></span>
                </div>
            </div>
        ...
        TMToolbar.render(document.getElementById('tcHeaderToolbar'), {
            ...
            rightActions: [
                ...(_isEditing ? [] : [{ id: 'run', label: '▶ Execute', onClick: () => TestCaseDetailView.runFromSuite() }]),
        ...
        TMSplitPane.mount(document.getElementById('tcSplitContainer'), {
            ...
            maxLeft: 400,
            leftHtml: `
                <div class="tm-case-detail__tree-shell">
                    <div class="tm-case-detail__tree-title">Test Case Tree</div>
                    <div id="tcTreeModeBtns" class="tm-case-detail__tree-actions"></div>
                </div>
                <div id="tcDetailTree"></div>
            `,
            rightHtml: `
                <div id="tcTabBarContainer"></div>
                <div id="tcDetailTabContent" class="tm-case-detail__tab-body"></div>
            `,
        });
    ...
    function _renderTree() {
        ...
        siblingContainer.className = 'tm-case-detail__group-list';
    ...
    function _renderGroup(label, items) {
        ...
        return `
            <div class="tm-case-detail__group">
                <div class="tm-case-detail__group-title ${selected ? 'is-selected' : ''}">${_esc(label)} <span class="tm-case-detail__group-count">(${items.length})</span></div>
                <div class="tm-case-detail__group-items">
                    ${items.slice(0, 10).map(tc => `
                        <button class="btn btn-sm tm-case-detail__group-link ${Number(tc.id) === Number(_caseId) ? 'btn-primary' : 'btn-secondary'}" onclick="App.navigate('test-case-detail', ${Number(tc.id)})">
                            ${_esc(tc.code || `TC-${tc.id}`)}
                        </button>
                    `).join('')}
                    ${items.length > 10 ? `<div class="tm-case-detail__group-more">+${items.length - 10} more</div>` : ''}
                </div>
            </div>
        `;
    }
    ...
    function _linkedSuitesForCurrentCase() {
        const suiteIds = new Set((Array.isArray(_tc?.suite_ids) ? _tc.suite_ids : []).map(Number));
        return (_suites || []).filter((suite) => suiteIds.has(Number(suite.id)));
    }
    ...
    function startEdit() {
        ...
        _draft = {
            ..._tc,
            suite_ids: Array.isArray(_tc.suite_ids) ? [..._tc.suite_ids] : [],
        };
    ...
    async function saveEdit() {
        ...
        _draftSteps = [];
        _execLoaded = false;
        await _loadData(TestingShared.getProgram(), _caseId);
        _renderShell();
        _renderTree();
        _renderTab();
    }
    ...
    async function _renderTab() {
        ...
        if (_activeTab === 'details') {
            el.innerHTML = `<div id="tcApprovalBanner" class="tm-case-detail__approval"></div><div id="tcPropPanel"></div><div id="tcSuiteSection" class="tm-case-detail__suite"></div>`;
            ...
            const suiteIds = new Set((Array.isArray(current.suite_ids) ? current.suite_ids : []).map(Number));
            ...
            if (_isEditing) {
                suiteSec.innerHTML = `<div class="tm-case-detail__suite-heading">Suite Assignments</div><div id="tcSuiteDropdown"></div>`;
                ...
                suiteSec.innerHTML = `<div class="tm-case-detail__suite-heading">Suite Assignments</div>
                    <div class="tm-case-detail__suite-badges">${linked.length ? linked.map(s => TMStatusBadge.html(s.name || `Suite #${s.id}`, {customBg:'#e8f0fe',customFg:'#174ea6'})).join('') : '<span class="tm-muted">No suites assigned</span>'}</div>`;
            }
            return;
        }
        ...
        if (_activeTab === 'executions') {
            ...
            const linkedSuites = _linkedSuitesForCurrentCase();
            el.innerHTML = `
                <div class="tm-inline-actions tm-block--md">
                    <button class="btn btn-primary btn-sm" ${linkedSuites.length ? '' : 'disabled title="Assign this test case to a suite first"'} onclick="TestCaseDetailView.runFromSuite()">▶ Execute This Test Case</button>
                    <span class="tm-muted">${linkedSuites.length
                        ? (linkedSuites.length === 1
                            ? `Uses linked suite: ${_esc(linkedSuites[0].name || `Suite #${linkedSuites[0].id}`)}`
                            : `Choose one of ${linkedSuites.length} linked suites for execution`)
                        : 'Assign this test case to a suite before execution.'}</span>
                </div>
                <div id="tcExecGrid"></div>
            `;
        ...
        if (_activeTab === 'defects') {
            ...
            el.innerHTML = `
                <div class="tm-case-detail__defects-block">
                    <div class="tm-case-detail__section-title">Linked Defects</div>
                    <div id="tcLinkedDefectsGrid"></div>
                </div>
                <div>
                    <div class="tm-case-detail__section-title">Link Existing Defect</div>
            ...
            TMDataGrid.render(document.getElementById('tcLinkedDefectsGrid'), {
                columns: [
                    ...
                    { key: '_actions', label: '', width: '70px', render: (d) => `<button class="tm-toolbar__btn tm-case-detail__mini-btn" onclick="TestCaseDetailView.unlinkDefect(${d.id})">Unlink</button>` },
                ],
                rows: linked,
            ...
            TMDataGrid.render(document.getElementById('tcPoolDefectsGrid'), {
                columns: [
                    ...
                    { key: 'title', label: 'Title' },
                    { key: 'status', label: 'Status', width: '90px', render: (d) => TMStatusBadge.html(d.status || '-') },
                    { key: '_actions', label: '', width: '70px', render: (d) => `<button class="tm-toolbar__btn tm-toolbar__btn--primary tm-case-detail__mini-btn" onclick="TestCaseDetailView.linkDefect(${d.id})">Link</button>` },
                ],
                rows: pool,
        ...
        el.innerHTML = `
            <div id="tcAuditPanel" class="tm-case-detail__audit"></div>
            <div class="tm-case-detail__section tm-case-detail__history-block">
                <div class="tm-case-detail__history-head">
                    <strong class="tm-case-detail__history-heading">Versions</strong>
                    <div class="tm-case-detail__history-actions">
                        <button class="tm-toolbar__btn" onclick="TestCaseDetailView.loadVersionDiff()">Compare</button>
                    </div>
                </div>
                ${_versions.length ? `
                    <div class="tm-case-detail__history-grid">
                        <div class="tm-case-detail__history-field">
                            <label>From</label>
                            <select class="tm-input" onchange="TestCaseDetailView.setVersionFrom(this.value)">
                        ...
                        <div class="tm-case-detail__history-field">
                            <label>To</label>
                            <select class="tm-input" onchange="TestCaseDetailView.setVersionTo(this.value)">
                ...
                ` : '<p class="tm-muted tm-case-detail__history-empty">No version snapshots yet.</p>'}
            </div>
            ${summary ? `
                <div class="tm-card tm-diff-viewer tm-case-detail__history-card">
                    <div class="tm-case-detail__history-head">
                        <strong class="tm-case-detail__history-heading">Diff: v${_versionFrom} → v${_versionTo}</strong>
                        <div class="tm-case-detail__history-summary">
                            <span class="tm-badge tm-case-detail__badge--fields">Fields: ${summary.field_change_count}</span>
                            <span class="tm-badge tm-case-detail__badge--added">+${summary.step_added_count} steps</span>
                            <span class="tm-badge tm-case-detail__badge--removed">-${summary.step_removed_count} steps</span>
                            <span class="tm-badge tm-case-detail__badge--changed">Δ${summary.step_changed_count} steps</span>
                        </div>
                    </div>
                    ...
                    <div class="tm-diff-section">
                        ...
                        <table class="tm-diff-table">
                            <thead>
                                <tr>
                                    <th class="tm-case-detail__field-col">Field</th>
                    ...
                    ` : '<div class="tm-muted tm-case-detail__history-note">No field-level changes.</div>'}
                    ...
                    <div class="tm-diff-section">
                        ...
                        <table class="tm-diff-table">
                            <thead>
                                <tr>
                                    <th class="tm-case-detail__step-col">Step</th>
                                    <th class="tm-diff-col-old">v${_versionFrom} (Old)</th>
                                    <th class="tm-diff-col-new">v${_versionTo} (New)</th>
                                </tr>
                            </thead>
            ...
            ` : ''}
            <div>
                <strong class="tm-case-detail__recent-title">Recent Activity</strong>
                <div id="tcRecentActivityGrid" class="tm-case-detail__recent-grid"></div>
            </div>
        `;
        ...
        if (_versions.length) {
            TMDataGrid.render(document.getElementById('tcVersionGrid'), {
                columns: [
                    ...
                    { key: '_actions', label: '', width: '70px', render: (v) => `<button class="tm-toolbar__btn tm-case-detail__mini-btn" onclick="TestCaseDetailView.restoreVersion(${v.version_no})">Restore</button>` },
                ],
                rows: _versions,
            });
        }
    ...
    async function runFromSuite() {
        const linkedSuites = _linkedSuitesForCurrentCase();
        if (!linkedSuites.length) {
            App.toast('Assign this test case to a suite before execution.', 'warning');
            return;
        }
        if (linkedSuites.length === 1) {
            await runViaSuite(linkedSuites[0].id);
            return;
        }

        App.openModal(`
            <div class="modal-header">
                <h2>Choose Suite to Execute</h2>
                <button class="modal-close" onclick="App.closeModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p class="tm-muted">This test case belongs to multiple suites. Pick the suite that should create the execution cycle.</p>
                <div class="tm-selection-shell">
                    ${linkedSuites.map((suite) => `
                        <button class="btn btn-secondary tm-block tm-block--sm" style="width:100%;justify-content:flex-start" onclick="TestCaseDetailView.runViaSuite(${suite.id})">
                            ▶ ${_esc(suite.name || `Suite #${suite.id}`)}
                        </button>
                    `).join('')}
                </div>
            </div>
    ...
    async function runViaSuite(suiteId) {
        const targetSuiteId = Number(suiteId);
        if (!targetSuiteId) return;
        App.closeModal();

        if (typeof TestPlanningView !== 'undefined' && typeof TestPlanningView.runSuiteQuick === 'function') {
            await TestPlanningView.runSuiteQuick(targetSuiteId);
            return;
        }

        App.navigate('test-planning');
        setTimeout(() => {
            if (typeof TestPlanningView !== 'undefined' && typeof TestPlanningView.runSuiteQuick === 'function') {
                TestPlanningView.runSuiteQuick(targetSuiteId);
                return;
            }
            App.toast('Quick Run is not available right now.', 'error');
        }, 150);
    }
