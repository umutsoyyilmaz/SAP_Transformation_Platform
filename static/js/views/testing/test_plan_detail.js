...
const TestPlanDetailView = (() => {
    ...
    async function _onScopeTypeChange() {
        ...
        if ((scopeType === 'l3_process' || scopeType === 'scenario') && !projectId) {
            area.innerHTML = '<p style="color:#999;font-size:13px">Select an active project to load Explore scope items.</p>';
            return;
        }
        try {
            let items = [];
            if (scopeType === 'requirement') {
                ...
                try {
                    const res = await API.get(`/explore/process-levels?project_id=${projectId}&level=3&flat=true&per_page=200`);
                ...
                try {
                    const res = await API.get(`/explore/workshops?project_id=${projectId}&per_page=200`);
    ...
    async function deleteScope(id) {
        const confirmed = await App.confirmDialog({
            ...
            message: 'Remove this scope item?',
            confirmLabel: 'Remove',
            variant: 'primary',
            testId: 'test-plan-delete-scope-modal',
            confirmTestId: 'test-plan-delete-scope-submit',
            cancelTestId: 'test-plan-delete-scope-cancel',
        });
        if (!confirmed) return;
    ...
    async function refreshCoverage() {
        ...
        try {
            await API.post(`/testing/plans/${_planId}/coverage/refresh`, {});
    ...
    async function removeTC(ptcId) {
        const confirmed = await App.confirmDialog({
            ...
            message: 'Remove this test case from the plan?',
            confirmLabel: 'Remove',
            variant: 'primary',
            testId: 'test-plan-remove-tc-modal',
            confirmTestId: 'test-plan-remove-tc-submit',
            cancelTestId: 'test-plan-remove-tc-cancel',
        });
        if (!confirmed) return;
        ...
        list.innerHTML = suites.map(s => `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;border-bottom:1px solid #f0f0f0">
                <div>
                    ...
                    <span class="badge" style="background:#eee;color:#666;margin-left:6px">${esc(s.purpose || 'general')}</span>
    ...
    async function unlinkDataSet(pdsId) {
        const confirmed = await App.confirmDialog({
            title: 'Unlink Data Set',
            message: 'Unlink this data set from the plan?',
            confirmLabel: 'Unlink',
            variant: 'primary',
            testId: 'test-plan-unlink-dataset-modal',
            confirmTestId: 'test-plan-unlink-dataset-submit',
            cancelTestId: 'test-plan-unlink-dataset-cancel',
        });
        if (!confirmed) return;
    ...
    async function _renderCyclesTab(c) {
        ...
        const cycles = _plan.cycles || [];
        const pid = TestingShared.getProgram();
        const readinessPayload = pid ? await API.get(`/programs/${pid}/testing/release-readiness`).catch(() => ({ items: [] })) : { items: [] };
        const readinessByCycle = new Map((readinessPayload.items || []).map((row) => [Number(row.cycle_id), row]));
        ...
        c.innerHTML = `
            ...
            ${cycles.length === 0
                ...
                : `<table class="data-table">
                    <thead><tr>
                        <th>Cycle</th><th>Layer</th><th>Environment</th><th>Build / Transport</th><th>Owner</th><th>Readiness</th><th>Status</th><th>Start</th><th>End</th><th>Actions</th>
                    </tr></thead>
                    <tbody>
                        ${cycles.map(cy => {
                            const readiness = readinessByCycle.get(Number(cy.id));
                            const readinessLabel = readiness?.readiness || 'unknown';
                            const buildTransport = [readiness?.build_tag || cy.build_tag || '—', readiness?.transport_request || cy.transport_request || '—'].join(' / ');
                            return `<tr>
                            <td><strong>${esc(cy.name)}</strong></td>
                            <td>${esc((cy.test_layer || '—').toUpperCase())}</td>
                            <td>${esc(cy.environment || '—')}</td>
                            <td>${esc(buildTransport)}</td>
                            <td>${esc(readiness?.owner || cy.owner_member?.name || '—')}</td>
                            <td><span class="badge" style="background:${readinessLabel === 'ready_now' ? '#107e3e' : '#b45309'};color:#fff">${esc(readinessLabel)}</span></td>
    ...
    async function populateFromPlan(cycleId) {
        const confirmed = await App.confirmDialog({
            title: 'Populate Cycle',
            message: 'Populate this cycle from the test plan? Execution records will be created for all planned test cases.',
            confirmLabel: 'Populate',
            variant: 'primary',
            testId: 'test-plan-populate-cycle-modal',
            confirmTestId: 'test-plan-populate-cycle-submit',
            cancelTestId: 'test-plan-populate-cycle-cancel',
        });
        if (!confirmed) return;
        try {
    ...
    async function _deleteCycle(cycleId) {
        const confirmed = await App.confirmDialog({
            title: 'Delete Cycle',
            message: 'Delete this cycle and all its executions?',
            confirmLabel: 'Delete',
            testId: 'test-plan-delete-cycle-modal',
            confirmTestId: 'test-plan-delete-cycle-submit',
            cancelTestId: 'test-plan-delete-cycle-cancel',
        });
        if (!confirmed) return;
