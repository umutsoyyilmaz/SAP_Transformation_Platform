...
const ApprovalsView = (function () {
    ...
    async function _loadPending() {
        const pid = (typeof App !== 'undefined' && App.getActiveProgram)
            ? App.getActiveProgram()?.id
            : null;
        const url = pid
    ...
    async function render() {
        ...
        main.innerHTML = `
            <div class="pg-view-header">
                ${PGBreadcrumb.html([{ label: 'Programs', onclick: 'App.navigate("programs")' }, { label: 'Approvals & Sign-off' }])}
                <h2 class="pg-view-title">Approvals & Sign-off</h2>
            </div>
            ${TestingShared.renderModuleNav('signoff-approvals')}
    ...
    function _renderPending(el) {
        const canDecide = TestingShared.canPerform('approval_decide');
        ...
        TMDataGrid.render(document.getElementById('approvalGrid'), {
            columns: [
                ...
                { key: '_actions', label: 'Actions', width: '180px', render: (r) => canDecide ? `
                ...
                ` : `<span class="tm-muted">Role required</span>` },
            ],
    ...
    async function decide(recordId, decision) {
        if (!TestingShared.canPerform('approval_decide')) {
            App.toast('Operational approval role required', 'warning');
            return;
        }
        let comment = '';
        if (decision === 'rejected') {
            comment = await App.promptDialog({
                title: 'Reject Approval',
                message: 'Rejection reason (optional)',
                label: 'Reason',
                placeholder: 'Add a rejection reason',
                confirmLabel: 'Reject',
                testId: 'approvals-reject-modal',
                multiline: true,
            });
    ...
    async function renderStatusBanner(containerEl, entityType, entityId) {
        ...
        containerEl.dataset.approvalEntityType = entityType;
        containerEl.dataset.approvalEntityId = String(entityId);
        await TestingShared.ensureOperationalPermissions();
        const canSubmit = TestingShared.canPerform('approval_submit');
            ...
            if (data.status === 'not_submitted') {
                containerEl.innerHTML = `
                    <div class="tm-approval-banner tm-approval-banner--draft">
                        ...
                        ` : `<span class="tm-muted">Operational submit role required</span>`}
                    </div>`;
                return;
            }
    ...
    async function submitEntity(entityType, entityId) {
        if (!TestingShared.canPerform('approval_submit')) {
            App.toast('Operational approval submit role required', 'warning');
            return;
        }
        try {
            ...
            const banner = document.querySelector(
                `[data-approval-entity-type="${entityType}"][data-approval-entity-id="${entityId}"]`,
            );
            if (banner) {
