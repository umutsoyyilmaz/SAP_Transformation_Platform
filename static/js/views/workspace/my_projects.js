...
const MyProjectsView = (() => {
    ...
    async function _openProject(item, destination = 'project-setup') {
                ...
                ${sorted.map((p) => `
                    <div class="program-card" style="cursor:default">
                        ...
                        <div class="program-card__actions">
                            ...
                            <button class="btn btn-primary btn-sm" onclick="MyProjectsView.openProject(${Number(p.project_id)})">Open Setup</button>
                        </div>
                    </div>
                `).join('')}
            </div>
            ...
            recentWrap.innerHTML = recent.length
                ? recent.map((r) => `
                    <button class="btn btn-secondary btn-sm" onclick="MyProjectsView.openProject(${Number(r.project_id)})">
                        Open Setup: ${escHtml(r.program_name)} / ${escHtml(r.project_name)}
                    </button>
                `).join(' ')
    ...
    async function render() {
        ...
        main.innerHTML = `
            <div class="pg-view-header">
                ...
                <p style="color:var(--sap-text-secondary);margin-top:4px">Use this launchpad to open project-owned setup workspaces. Only projects you are authorized to access are shown.</p>
            </div>
            <div class="card" style="margin-bottom:12px">
                <div style="margin-bottom:8px;color:var(--sap-text-secondary);font-size:13px">
                    <strong style="color:var(--sap-text-primary)">Project Setup</strong> is the canonical bootstrap cockpit for methodology, phases, team, workstreams, committees, and hierarchy.
                </div>
