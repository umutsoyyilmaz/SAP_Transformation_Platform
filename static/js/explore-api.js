...
const ExploreAPI = (() => {
    ...
    const levels = {
        listTree: (projectId, params) => API.get(`${B}/process-levels?project_id=${projectId}${_qs(params)}`),
        listChildren: (projectId, parentId, params) =>
            API.get(`${B}/process-levels?project_id=${projectId}&parent_id=${encodeURIComponent(parentId)}${_qs(params)}`),
        listL1:   (projectId) => _items(API.get(`${B}/process-levels?project_id=${projectId}&level=1`)),
        getL1:    (projectId, id) => API.get(`${B}/process-levels/${id}`),
        listL2:   (projectId) => _items(API.get(`${B}/process-levels?project_id=${projectId}&level=2`)),
        getL2:    (projectId, id) => API.get(`${B}/process-levels/${id}`),
        listL3:   (projectId) => _items(API.get(`${B}/process-levels?project_id=${projectId}&level=3`)),
        getL3:    (projectId, id) => API.get(`${B}/process-levels/${id}`),
        listL4:   (projectId) => _items(API.get(`${B}/process-levels?project_id=${projectId}&level=4`)),
        getL4:    (projectId, id) => API.get(`${B}/process-levels/${id}`),
        create:         (projectId, data) => API.post(`${B}/process-levels`, Object.assign({project_id: projectId}, data)),
        bulkCreate:     (projectId, levels, data) => API.post(`${B}/process-levels/bulk`, Object.assign({project_id: projectId, levels}, data || {})),
        remove:         (id, confirm, data) => {
            const params = new URLSearchParams();
            if (confirm) params.set('confirm', 'true');
            if (data?.mutation_context) params.set('mutation_context', data.mutation_context);
            const suffix = params.toString() ? `?${params.toString()}` : '';
            return API.delete(`${B}/process-levels/${id}${suffix}`);
        },
        importTemplate: (projectId, data) => API.post(`${B}/process-levels/import-template`, Object.assign({project_id: projectId}, data)),
        update:        (id, data) => API.put(`${B}/process-levels/${id}`, data),
        scopeMatrix:   (projectId, params) => API.get(`${B}/scope-matrix?project_id=${projectId}${_qs(params)}`),
        ...
        seedFromCatalog: (l3Id, data)   => API.post(`${B}/process-levels/${l3Id}/seed-from-catalog`, data || {}),
        addChild:        (l3Id, data) => API.post(`${B}/process-levels/${l3Id}/children`, data),
        l2Readiness:   (projectId) => API.get(`${B}/process-levels/l2-readiness?project_id=${projectId}`),
        areaMilestones: (projectId) => API.get(`${B}/area-milestones?project_id=${projectId}`),
    ...
    const workshops = {
        ...
        get:      (pid, id)     => API.get(`${B}/workshops/${id}?project_id=${pid}`),
        full:     (pid, id)     => API.get(`${B}/workshops/${id}/full?project_id=${pid}`),
        create:   (pid, data)   => API.post(`${B}/workshops`, Object.assign({project_id: pid}, data)),
        update:   (pid, id, d)  => API.put(`${B}/workshops/${id}`, Object.assign({project_id: pid}, d || {})),
        delete:   (pid, id)     => API.delete(`${B}/workshops/${id}?project_id=${pid}`),
        start:    (pid, id)     => API.post(`${B}/workshops/${id}/start`, { project_id: pid }),
        complete: (pid, id)     => API.post(`${B}/workshops/${id}/complete`, { project_id: pid }),
        transition: (pid, id, d) => {
            const action = d && d.action;
            const payload = Object.assign({ project_id: pid }, d || {});
            if (action === 'complete') return API.post(`${B}/workshops/${id}/complete`, payload);
            if (action === 'reopen') return API.post(`${B}/workshops/${id}/reopen`, payload);
            return API.post(`${B}/workshops/${id}/start`, payload);
        },
        capacity: (pid)         => API.get(`${B}/workshops/capacity?project_id=${pid}`),
        stats:    (pid)         => API.get(`${B}/workshops/stats?project_id=${pid}`),
        reopen:   (pid, id, data) => API.post(`${B}/workshops/${id}/reopen`, Object.assign({ project_id: pid }, data || {})),
        createDelta: (pid, id, data)  => API.post(`${B}/workshops/${id}/create-delta`, Object.assign({ project_id: pid }, data || {})),
        steps:    (pid, wsId)   => _items(API.get(`${B}/workshops/${wsId}/steps?project_id=${pid}`)),
    };
    ...
    const sessions = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/sessions?project_id=${pid}`),
        get:      (pid, wsId, id)    => API.get(`${B}/workshops/${id}?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/workshops/${wsId}/create-delta`, Object.assign({ project_id: pid }, data || {})),
        update:   (pid, wsId, id, d) => API.put(`${B}/workshops/${id}`, Object.assign({ project_id: pid }, d || {})),
    };
    ...
    const fitDecisions = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/fit-decisions?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/workshops/${wsId}/fit-decisions`, Object.assign({ project_id: pid }, data || {})),
        update:   (pid, wsId, stepId, d) => API.put(`${B}/process-steps/${stepId}`, d),
    };

    // ── Decisions ─────────────────────────────────────────────────
    const decisions = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/decisions?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/process-steps/${data.process_step_id}/decisions`, Object.assign({ project_id: pid }, data || {})),
    ...
    // ── Agenda Items ──────────────────────────────────────────────
    const agenda = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/agenda-items?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/workshops/${wsId}/agenda-items`, Object.assign({ project_id: pid }, data || {})),
    ...
    // ── Attendees ────────────────────────────────────────────────
    const attendees = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/attendees?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/workshops/${wsId}/attendees`, Object.assign({ project_id: pid }, data || {})),
    ...
    const dependencies = {
        list:     (pid, wsId)        => API.get(`${B}/workshops/${wsId}/dependencies?project_id=${pid}`),
        create:   (pid, wsId, data)  => API.post(`${B}/workshops/${wsId}/dependencies`, Object.assign({ project_id: pid }, data || {})),
    ...
    const documents = {
        list:            (pid, wsId) => API.get(`${B}/workshops/${wsId}/documents?project_id=${pid}`),
        generate:        (pid, wsId, data) => API.post(`${B}/workshops/${wsId}/documents/generate`, Object.assign({ project_id: pid }, data || {})),
        generateMinutes: (pid, wsId, data) => API.post(`${B}/workshops/${wsId}/generate-minutes`, Object.assign({ project_id: pid }, data || {})),
        generateSummary: (pid, wsId, data) => API.post(`${B}/workshops/${wsId}/ai-summary`, Object.assign({ project_id: pid }, data || {})),
    };
    ...
    // ── Process Steps ─────────────────────────────────────────────
    const processSteps = {
        list:           (pid, wsId)    => _items(API.get(`${B}/workshops/${wsId}/steps?project_id=${pid}`)),
