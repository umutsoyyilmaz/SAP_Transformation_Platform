...
**Status:** Execution Complete (Historical Plan Artifact)
**Date:** 2026-02-18
...
> Historical note: the live platform has progressed beyond this transition snapshot.
> Current runtime state uses `suite_ids` + `test_case_suite_links`, and both `suite_type` and `TestCase.suite_id` have been removed from the active contract.
...
| `create_test_case` | `app/blueprints/testing/catalog.py` | Accepts `suite_id`, `process_level_id` | **D1+D2:** Add resolve + validate + junction |
| `update_test_case` | `app/blueprints/testing/catalog.py` | `setattr` loop for all fields | **D2:** Add junction support |
...
| Suite picker (FE) | `static/js/views/testing/test_planning.js` | Single `<select>` | **D2:** Multi-select chips |
...
#### 1.8 DB Reset
```bash
.venv/bin/python scripts/db/reset_db_from_models.py
.venv/bin/python scripts/data/seed/seed_demo_data.py
```

**Verification:** Run `tests/test_management/test_api_testing.py` — all 207 tests must pass.
...
**Files:**
- `app/blueprints/testing/catalog.py` — Add 4 new routes
...
**Files:**
- `app/blueprints/testing/analytics.py` — Add 1 new route
...
**Files:**
- `static/js/views/testing/test_planning.js` — Suite picker → multi-select + coverage hints
...
- `app/blueprints/testing/catalog.py` — Remove `suite_id` writes from new code paths
- `tests/test_management/test_api_testing.py` — Add deprecation-aware assertions
...
|-----------|-------|-----------|--------|
| `tests/project_scope/test_scope_resolution.py` | `TestScopeResolution` | 12 | D1: L3 resolution chain (§7.1) |
| `tests/test_management/test_api_testing.py` | `TestSuiteNM` | 7 | D2: N:M junction CRUD (§7.2) |
| `tests/test_management/test_api_testing.py` | `TestScopeCoverage` | 5 | D1: L3 coverage endpoint (§7.3) |
| `tests/test_management/test_api_testing.py` | `TestL3Validation` | 4 | D1: CREATE validation |
| **Total** | | **28** | |
...
|-----------|--------------|------------------|-----|
| `tests/test_management/test_api_testing.py` | 207 | ~5 (create TC tests that don't provide L3) | Add `process_level_id` to test fixtures, or mark test_layer as "performance" |
| `tests/quality/test_demo_flow.py` | varies | ~2 (generate tests) | Update to check junction links |
