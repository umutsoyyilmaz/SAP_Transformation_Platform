...
Platform, tutarlı görsel dil icin CSS Custom Properties tabanli design token sistemi kullanir.

**Font Ailesi:**
- UI: Inter (Google Fonts CDN — 400/500/600/700 weights)
- Mono: JetBrains Mono (code bloklari, process kodlari, terminal ciktilari)
...
Component CSS           → Token referanslari (font-size: var(--fs-sm))
JS inline styles        → px degerler (scale ile uyumlu: 10, 11, 12, 13, 14px)
```

**Kurallar:**
1. CSS'de rem birimi kullanilmaz — tum font-size'lar var() veya px
2. Yeni component eklerken type scale disinda font-size tanimlanmaz
3. Font-family her zaman `var(--font-family)` veya `inherit` uzerinden kullanilir
4. JS inline style'larda font-size sadece scale'deki px degerleri kullanir
