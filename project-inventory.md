...
| API Routes | 321 (12 blueprint) |
| DB Modeli (class) | 74 (14 domain modül) |
| DB Tablosu (runtime) | 71 |
| Alembic Migration | 10 |
| Pytest Test | 860 (16 test dosyası) |
| AI Asistan | 3 aktif (NL Query, Requirement Analyst, Defect Triage) |
...
| D6 | [PROGRESS_REPORT.md](PROGRESS_REPORT.md) | Progress Report | — | 2026-02-10 | 562 | TR | 321 route, 71 tablo, 848 test — commit geçmişi + sprint durumu |
...
| D12 | [FRONTEND_DECISION.md](FRONTEND_DECISION.md) | Decision | ✅ Approved | 2026-02-10 | 281 | EN | Vue 3 incremental migration kararı, 4 faz planı |
...
| M6 | [app/models/testing.py](app/models/testing.py) | 1,151 | 14 | 14 | TestPlan, TestCycle, TestCase, TestExecution, Defect, TestSuite, TestStep, TestCaseDependency, TestCycleSuite, TestRun, TestStepResult, DefectComment, DefectHistory, DefectLink |
...
| B6 | [app/blueprints/testing_bp.py](app/blueprints/testing_bp.py) | 1,667 | 55 | Test Plans/Cycles/Cases/Runs/Executions, Defects, Suites, Steps, Comments, History, Links |
...
> **Not:** 321 registred route (1 static route dahil).
...
| A2 | [app/ai/gateway.py](app/ai/gateway.py) | 622 | LLM Gateway (3 provider + stub), token tracking |
...
| A7 | [app/ai/suggestion_queue.py](app/ai/suggestion_queue.py) | 227 | HITL suggestion lifecycle |
| | **Toplam** | **3,065** | |
...
| T2 | [tests/test_api_testing.py](tests/test_api_testing.py) | 1,438 | 147 | TestPlan/Cycle/Case/Execution/Defect/Suite/Step/Run/StepResult/Comment/History/Link |
...
| C11 | .env / .env.example | 32/26 | Environment değişkenleri |
...
test-management-fs-ts.md (D2)
  ├── Modeller   → app/models/testing.py (M6) — 14/17 tablo (TS-Sprint 1+2 done)
  ├── API        → app/blueprints/testing_bp.py (B6) — 55 route (hedef 45)
  ├── Frontend   → static/js/views/testing.js (F2)
  ├── Seed       → scripts/seed_data/specs_testing.py (SC2)
  ├── Migration  → MIG9 + MIG10
  ├── Test       → tests/test_api_testing.py (T2) — 147 test ✅
...
SAP_Platform_Project_Plan.md (D5) — ANA PLAN
  ...
  └──► FRONTEND_DECISION.md (D12) — Vue 3 karar
...
app/models/testing.py     → app/blueprints/testing_bp.py     → tests/test_api_testing.py      (147 test)
...
### 5.2 Bak Dosya Temizliği

| Dosya | Açıklama | Aksiyon |
|-------|----------|--------|
| app/blueprints/requirement_bp.py.bak | Backup dosyası | Silmek |
| app/blueprints/scope_bp.py.bak | Backup dosyası | Silmek |
| app/models/requirement.py.bak | Backup dosyası | Silmek |
| app/models/scope.py.bak | Backup dosyası | Silmek |
| scripts/seed_demo_data.py.bak | Backup dosyası | Silmek |
| tests/test_api_requirement.py.bak | Backup dosyası | Silmek |
| tests/test_api_scope.py.bak | Backup dosyası | Silmek |
...
| Test Mgmt: 17 tablo | 14 tablo | **82%** | UATSignOff, PerfTestResult, TestDailySnapshot (TS-Sprint 3) |
